...
class LoginUnlockScreen extends ConsumerStatefulWidget {
  ...
  /// Indica que la biometría falló y debe mostrarse la alternativa con PIN.
...
class _LoginUnlockScreenState extends ConsumerState<LoginUnlockScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final showKeypad =
        widget.canUsePin && (_showPinPadLocal || widget.biometricReady);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: widget.biometricLoading
                ? null
                : () {
                    if (biometricIcon != null) {
                      widget.onUnlockWithBiometric();
                    } else if (widget.canUsePin) {
            ...
            label: Text(
              biometricIcon != null
                  ? 'Desbloquear con $label'
                  : widget.canUsePin
                  ? 'Iniciar sesión con PIN'
                  : 'Iniciar sesión con contraseña',
            ),
          ),
          if (widget.pinError.isNotEmpty && !widget.canUsePin) ...[
          ...
          if (biometricIcon != null && widget.canUsePin)
            TextButton.icon(
              onPressed: widget.biometricLoading
                  ? null
                  : () => setState(() => _showPinPadLocal = true),
          ...
          if (biometricIcon != null && !widget.canUsePin)
            TextButton.icon(
              onPressed: widget.biometricLoading ? null : widget.onUsePassword,
              icon: const Icon(Icons.password_rounded, size: 16),
              label: const Text('Iniciar con contraseña'),
              style: TextButton.styleFrom(
                foregroundColor: AppColors.ink2,
                textStyle: const TextStyle(fontWeight: FontWeight.w700),
              ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Inicia una sesión nueva con tu PIN de 6 dígitos.',
            textAlign: TextAlign.center,
            style: TextStyle(
              color: AppColors.ink2,
              fontSize: 13,
          ...
          TextButton(
            onPressed: widget.pinSaving || widget.biometricLoading
                ? null
                : widget.onUsePassword,
            child: const Text(
