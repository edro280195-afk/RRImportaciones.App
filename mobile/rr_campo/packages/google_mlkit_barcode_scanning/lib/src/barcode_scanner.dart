...
/// Result of a barcode scan, including an optional zoom suggested by ML Kit.
class BarcodeScanResult {
  /// Creates a barcode scan result.
  const BarcodeScanResult({required this.barcodes, this.zoomSuggestion});

  /// Decoded and potential barcodes found in the image.
  final List<Barcode> barcodes;

  /// Camera zoom ratio suggested by ML Kit on Android.
  final double? zoomSuggestion;
}
...
class BarcodeScanner {
  ...
  /// Whether Android ML Kit should return potential undecoded barcodes.
  final bool enableAllPotentialBarcodes;

  /// Maximum camera zoom ratio Android ML Kit can suggest.
  ...
  BarcodeScanner({
    ...
    this.enableAllPotentialBarcodes = false,
    this.maxZoomRatio,
  }) : assert(maxZoomRatio == null || maxZoomRatio > 1);
  ...
  Future<List<Barcode>> processImage(InputImage inputImage) async =>
      (await processImageWithResult(inputImage)).barcodes;

  /// Processes [inputImage] and returns barcodes plus ML Kit's zoom suggestion.
  Future<BarcodeScanResult> processImageWithResult(
    InputImage inputImage,
  ) async {
    final dynamic result = await _channel
        .invokeMethod('vision#startBarcodeScanner', {
    ...
    final dynamic rawBarcodes = result is Map ? result['barcodes'] : result;
    final barcodesList = <Barcode>[];
    if (rawBarcodes is List) {
      for (final dynamic json in rawBarcodes) {
        barcodesList.add(Barcode.fromJson(json));
      }
    }

    final dynamic rawZoomSuggestion = result is Map
        ? result['zoomSuggestion']
        : null;
    return BarcodeScanResult(
      barcodes: barcodesList,
      zoomSuggestion: rawZoomSuggestion is num
          ? rawZoomSuggestion.toDouble()
          : null,
    );
  }
