    ...
    >
      ...
      <div class="flex items-center gap-3 px-5 py-4 bg-white border-b border-[#F0F2F5] shrink-0">
        ...
        <div class="min-w-0">
          ...
          <p class="text-[11px] text-[#8B93A1] truncate">Asistente IA &middot; R&amp;R Importaciones</p>
        </div>
          ...
          <div class="flex items-center gap-1 bg-[#F5F6F8] rounded-lg p-0.5">
            @for (p of providers(); track p.id) {
              <button
                ...
                [title]="p.hasTools ? 'Con herramientas (cotizaciones, consultas)' : 'Solo chat simple'"
              >{{ p.label }}</button>
            }
          </div>
        }
        ...
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-[14px] h-[14px] stroke-[1.8]">
                ...
                @if (msg.toolCalls && msg.toolCalls.length > 0 && !msg.error) {
                  <div class="mt-1 flex flex-wrap gap-1">
                    @for (tc of msg.toolCalls; track tc) {
                      <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#F0F2F5] text-[10px] text-[#6B7280] font-medium">
                ...
                @if (!msg.error && !msg.isWelcome) {
                  <div class="mt-1 flex items-center gap-1 ml-1">
                    <button
                      ...
                      [class]="msg.feedback === 'up' ? 'text-[#C61D26] bg-[#FEF2F2]' : 'text-[#B0B8C4] hover:text-[#6B7280] hover:bg-[#F0F2F5]'"
                    ...
                    <button
                      ...
                      class="w-6 h-6 rounded-md flex items-center justify-center transition-colors"
                      [class]="msg.feedback === 'down' ? 'text-[#C61D26] bg-[#FEF2F2]' : 'text-[#B0B8C4] hover:text-[#6B7280] hover:bg-[#F0F2F5]'"
      ...
      @if (messages().length === 1 && !loading()) {
        <div class="px-4 pb-3 bg-[#F8FAFC] space-y-3 shrink-0">
          @for (group of contextualSuggestions; track group.label) {
            <div>
              <p class="text-[11px] text-[#8B93A1] font-semibold uppercase tracking-[0.8px] mb-1.5">{{ group.label }}</p>
              <div class="space-y-1">
  ...
  private sendMessage(texto: string): void {
    ...
    this.rodriService.chat(texto, historial, this.provider()).subscribe({
      next: (res) => {
        ...
        const toolCalls = res.toolCallsEjecutados?.length
          ? res.toolCallsEjecutados
          : undefined;
