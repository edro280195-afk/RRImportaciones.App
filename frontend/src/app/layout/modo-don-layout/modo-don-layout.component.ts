...
@Component({
  ...
  template: `
    <div class="flex flex-col min-h-screen bg-[#F8FAFC]">
      <!-- Header minimalista -->
      <header
        class="fixed top-0 inset-x-0 h-14 z-20 bg-white border-b border-[#E5E7EB] flex items-center px-5 gap-3 shadow-sm"
      >
        <!-- Logo + nombre sistema -->
        <div class="flex items-center gap-2.5 shrink-0">
          <img src="assets/imagenes/rr_logo.png" class="h-7 w-auto" alt="R&R" />
          <span class="text-[14px] font-bold text-[#0D1017] hidden sm:inline"
            >R&amp;R Importaciones</span
          >
        </div>

        <div class="h-5 w-px bg-[#E5E7EB] hidden sm:block"></div>

        <span class="text-[13px] font-semibold text-[#C61D26] hidden sm:inline"
