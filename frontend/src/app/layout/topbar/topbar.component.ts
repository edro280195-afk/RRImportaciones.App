...
@Component({
  ...
  template: `
    ...
    >
      <button
        type="button"
        class="md:hidden grid h-9 w-9 place-items-center rounded-lg text-[var(--n-800)] hover:bg-[var(--n-100)]"
      ...
      <nav class="min-w-0 flex items-center gap-2 text-[12px] sm:text-[13px] text-[var(--n-500)] max-w-[36vw] sm:max-w-[260px]">
        @for (crumb of breadcrumbs(); track crumb; let last = $last) {
          @if (!last) {
            <span class="hidden sm:inline truncate max-w-[120px]">{{ crumb }}</span>
            <span class="hidden sm:inline text-[var(--n-300)]">/</span>
          } @else {
            <span class="text-[var(--n-800)] font-semibold truncate max-w-[82px] sm:max-w-[220px]">{{ crumb }}</span>
          }
        }
      </nav>
      ...
      <div class="relative ml-auto flex min-w-0 flex-1 justify-end md:justify-center">
        <button
          type="button"
          class="md:hidden w-9 h-9 rounded-lg flex items-center justify-center text-[var(--n-500)] hover:bg-[var(--n-100)] hover:text-[var(--n-800)] transition-all duration-150"
        ...
        <label
          class="hidden md:flex w-full max-w-[620px] min-w-[220px] items-center gap-[9px] bg-[var(--n-100)] border border-[var(--border)] rounded-lg px-3 py-2 transition-all duration-200 focus-within:border-[var(--rr-600)] focus-within:bg-[var(--bg-surface)] focus-within:shadow-[0_0_0_3px_rgba(198,29,38,0.10)] cursor-text"
          for="topbar-search-input"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-[14px] h-[14px] text-[var(--n-400)] shrink-0 stroke-2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
          </svg>
          <input
            id="topbar-search-input"
            type="text"
            placeholder="Buscar cliente, VIN, pedimento, módulo..."
            class="bg-transparent border-none outline-none text-[13px] w-full text-[var(--n-800)] placeholder:text-[var(--n-400)]"
            [value]="searchTerm()"
            (input)="setSearchTerm($event)"
            (focus)="openSearch()"
            autocomplete="off"
          />
          <kbd class="font-mono-data text-[10px] text-[var(--n-500)] bg-[var(--bg-surface)] px-1.5 py-0.5 rounded-[4px] border border-[var(--border)] shadow-sm leading-none">Ctrl K</kbd>
        </label>

        @if (searchOpen()) {
          <section class="fixed inset-x-3 top-[64px] max-h-[calc(100vh-80px)] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] shadow-[var(--shadow-xl)] md:absolute md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:top-[44px] md:w-[620px]">
            <div class="md:hidden p-3 border-b border-[var(--border)]">
              <label class="flex items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3 py-2">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-[15px] h-[15px] text-[var(--n-400)] stroke-2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <input
                  ...
                  class="w-full bg-transparent text-[14px] outline-none text-[var(--n-800)]"
                  [value]="searchTerm()"
                  (input)="setSearchTerm($event)"
                  autocomplete="off"
                />
            ...
            <div class="p-2">
              @for (action of filteredActions(); track action.route) {
                ...
                >
                  <span class="min-w-0">
                    <span class="block truncate text-[13px] font-semibold text-[var(--n-800)]">{{ action.label }}</span>
                    <span class="block truncate text-[12px] text-[var(--n-500)]">{{ action.detail }}</span>
                  </span>
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="h-4 w-4 shrink-0 text-[var(--n-400)] stroke-2">
                ...
                <div class="px-3 py-5 text-center">
                  @if (searchLoading()) {
                    <p class="text-[13px] font-semibold text-[var(--n-800)]">Buscando...</p>
                    <p class="mt-1 text-[12px] text-[var(--n-500)]">Revisando clientes, trámites, cotizaciones y pagos.</p>
                  } @else {
                    <p class="text-[13px] font-semibold text-[var(--n-800)]">Sin coincidencias</p>
                    <p class="mt-1 text-[12px] text-[var(--n-500)]">Prueba con cliente, trámite, pago o configuración.</p>
                  }
                </div>
              }
            </div>
      ...
      <div class="flex shrink-0 items-center gap-0.5 sm:gap-2">
        ...
        <div class="relative">
          <button
            type="button"
            class="w-9 h-9 rounded-lg flex items-center justify-center text-[var(--n-500)] hover:bg-[var(--n-100)] hover:text-[var(--n-800)] transition-all duration-150 relative"
          ...
          @if (notificationsOpen()) {
            <section class="fixed inset-x-3 top-[64px] max-h-[calc(100vh-80px)] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] shadow-[var(--shadow-xl)] md:absolute md:inset-x-auto md:right-0 md:top-[44px] md:w-[360px]">
              ...
              <div class="max-h-[340px] overflow-y-auto p-2">
                @for (item of visibleNotifications(); track item.title) {
                  <button type="button" class="flex w-full gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-[var(--n-50)] transition-colors" (click)="go(item.route)">
                    <span class="mt-[5px] h-2 w-2 shrink-0 rounded-full"
                      [style.background]="item.severity === 'warning' ? 'var(--amber)' : item.severity === 'info' ? 'var(--blue)' : 'var(--green)'"></span>
                    <span class="min-w-0">
                      <span class="block text-[13px] font-semibold text-[var(--n-800)]">{{ item.title }}</span>
                      <span class="mt-0.5 block text-[12px] leading-5 text-[var(--n-500)]">{{ item.detail }}</span>
                    </span>
                  </button>
                } @empty {
                  <div class="px-4 py-6 text-center">
                    <p class="text-[13px] font-semibold text-[var(--n-800)]">Sin alertas visibles</p>
                    <p class="mt-1 text-[12px] text-[var(--n-400)]">Tu rol no tiene pendientes disponibles aquí.</p>
                  </div>
                }
              </div>
            </section>
        ...
        <div class="relative">
          ...
          @if (settingsOpen()) {
            <section class="fixed inset-x-3 top-[64px] max-h-[calc(100vh-80px)] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] shadow-[var(--shadow-xl)] md:absolute md:inset-x-auto md:right-0 md:top-[44px] md:w-[340px]">
              <div class="border-b border-[var(--border)] px-4 py-3">
              ...
              @if (auth.isAdmin()) {
                <div class="p-2">
                  @for (item of adminSettings; track item.route) {
                    <button type="button" class="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-[var(--n-50)] transition-colors" (click)="go(item.route)">
                      <span>
                        <span class="block text-[13px] font-semibold text-[var(--n-800)]">{{ item.label }}</span>
                        <span class="mt-0.5 block text-[12px] text-[var(--n-400)]">{{ item.detail }}</span>
                      </span>
                      <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="h-4 w-4 shrink-0 text-[var(--n-400)] stroke-2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
                      </svg>
                ...
                <div class="px-4 py-5">
                  ...
                  <p class="mt-1 text-[12px] leading-5 text-[var(--n-500)]">Tu usuario no tiene permisos para cambiar configuración, permisos ni contraseñas.</p>
                </div>
              }
            </section>
          }
...
export class TopbarComponent implements OnInit {
  ...
  readonly adminSettings: QuickAction[] = [
    ...
    { label: 'Catálogo de precios', detail: 'Anexo 2 SAT — editar y limpiar entradas', route: '/admin/catalogo-precios' },
