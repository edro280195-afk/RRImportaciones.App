...
@Component({
  ...
  template: `
    ...
    >
      ...
      <nav
        class="min-w-0 flex items-center gap-2 text-[12px] sm:text-[13px] text-[var(--n-500)] max-w-[36vw] sm:max-w-[260px]"
      >
        @for (crumb of breadcrumbs(); track crumb; let last = $last) {
          @if (!last) {
            ...
            <span
              class="text-[var(--n-800)] font-semibold truncate max-w-[82px] sm:max-w-[220px]"
              >{{ crumb }}</span
            >
          }
        }
      ...
      <div class="relative ml-auto flex min-w-0 flex-1 justify-end md:justify-center">
        ...
        >
          <svg
            ...
            stroke="currentColor"
            viewBox="0 0 24 24"
            class="w-[14px] h-[14px] text-[var(--n-400)] shrink-0 stroke-2"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
          ...
          />
          <kbd
            class="font-mono-data text-[10px] text-[var(--n-500)] bg-[var(--bg-surface)] px-1.5 py-0.5 rounded-[4px] border border-[var(--border)] shadow-sm leading-none"
            >Ctrl K</kbd
          >
        </label>
        ...
        @if (searchOpen()) {
          <section
            class="fixed inset-x-3 top-[64px] max-h-[calc(100vh-80px)] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] shadow-[var(--shadow-xl)] md:absolute md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:top-[44px] md:w-[620px]"
          >
            <div class="md:hidden p-3 border-b border-[var(--border)]">
              <label
                class="flex items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3 py-2"
              >
                <svg
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  class="w-[15px] h-[15px] text-[var(--n-400)] stroke-2"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
            ...
            <div class="p-2">
              @for (action of filteredActions(); track action.route) {
                ...
                >
                  <span class="min-w-0">
                    <span class="block truncate text-[13px] font-semibold text-[var(--n-800)]">{{
                      action.label
                    }}</span>
                    <span class="block truncate text-[12px] text-[var(--n-500)]">{{
                      action.detail
                    }}</span>
                  </span>
                  <svg
                    ...
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    class="h-4 w-4 shrink-0 text-[var(--n-400)] stroke-2"
                  >
                ...
                <div class="px-3 py-5 text-center">
                  @if (searchLoading()) {
                    ...
                    <p class="mt-1 text-[12px] text-[var(--n-500)]">
                      Revisando clientes, trámites, cotizaciones y pagos.
                    </p>
                  } @else {
                    <p class="text-[13px] font-semibold text-[var(--n-800)]">Sin coincidencias</p>
                    <p class="mt-1 text-[12px] text-[var(--n-500)]">
                      Prueba con cliente, trámite, pago o configuración.
                    </p>
                  }
                </div>
              }
                  ...
                  @for (item of adminSettings; track item.route) {
                    ...
                    >
                      <span>
                        ...
                        <span class="mt-0.5 block text-[12px] text-[var(--n-400)]">{{
                          item.detail
                        }}</span>
                      </span>
                      <svg
...
export class TopbarComponent implements OnInit {
  ...
  private readonly quickActions: QuickAction[] = [
    {
      label: 'Clientes',
      detail: 'Buscar expedientes por cliente',
      route: '/clientes',
    ...
    {
      label: 'Tramites',
      detail: 'Operacion activa y seguimiento',
      route: '/tramites',
      permiso: 'TRAMITES_VER',
    },
    {
    ...
    },
    {
      ...
      detail: 'Crear una propuesta para cliente',
      route: '/cotizaciones/nueva',
      permiso: 'COTIZACIONES_CREAR',
    },
    {
      label: 'Cotizaciones',
      detail: 'Historico, conversion y vencimientos',
      route: '/cotizaciones',
      permiso: 'COTIZACIONES_VER',
    },
    {
      label: 'Pagos',
      detail: 'Cobros por verificar y comprobantes',
      route: '/pagos',
      permiso: 'PAGOS_VER',
    },
    {
    ...
    {
      label: 'Usuarios',
      detail: 'Cuentas del sistema y personal de campo',
      route: '/usuarios',
    ...
    {
      label: 'Auditoria',
      detail: 'Registro de cambios del sistema',
      route: '/auditoria',
      permiso: 'ADMIN_ONLY',
    },
  ];
  ...
  private readonly notifications: AppNotification[] = [
    {
      ...
      detail: 'Revisa comprobantes nuevos antes de conciliarlos.',
      route: '/pagos',
      severity: 'warning',
      permiso: 'PAGOS_VER',
    },
    {
      ...
      detail: 'Da seguimiento antes de que el cliente pierda vigencia.',
      route: '/cotizaciones',
      severity: 'warning',
      permiso: 'COTIZACIONES_VER',
    },
    {
      title: 'Tareas de campo abiertas',
  ...
  private runGlobalSearch(term: string): void {
    forkJoin({
      clientes: this.canSee('CLIENTES_VER')
        ? this.clienteService
            .getList({ search: term, page: 1, pageSize: 4 })
      ...
      cotizaciones: this.canSee('COTIZACIONES_VER')
        ? this.cotizacionService
            .getList({ search: term, page: 1, pageSize: 3 })
            .pipe(catchError(() => of({ items: [] })))
        : of({ items: [] }),
      ...
      .pipe(
        map(
          ({ clientes, tramites, cotizaciones, pagos }) =>
            [
  ...
  private updateBreadcrumbs(): void {
    ...
    this.breadcrumbs.set(
      path.length === 0 ? ['Inicio'] : path.map(s => labels[s] ?? this.capitalize(s))
    );
  }
