...
@Component({
  ...
  template: `
    <section class="confirm-backdrop" (click)="cancel.emit()">
      <article
        class="confirm-card"
        [ngClass]="'confirm-card--' + modal.severity"
        (click)="$event.stopPropagation()"
        role="dialog"
        aria-modal="true"
      >
        <div class="confirm-mark">
          {{ modal.severity === 'confirm' ? '?' : iconFor(modal.severity) }}
        </div>
        <div class="confirm-content">
          <div class="confirm-header">
            <h2>{{ modal.title }}</h2>
            <button type="button" class="confirm-close" (click)="cancel.emit()" aria-label="Cerrar">
              x
            </button>
          </div>
