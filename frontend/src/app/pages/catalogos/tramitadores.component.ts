              ...
              @for (t of tramitadores(); track t.id) {
                <tr class="border-b border-[#F0F2F5] hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <td class="px-4 py-3 text-[#6B717F]">
                    @if (t.comisionTipo === 'NA') {
                      <span class="text-[#8B93A1]">Sin comisión</span>
                    } @else {
                      {{ t.comisionTipo }} {{ t.comisionValor }}
                    }
                  </td>
                  <td class="px-4 py-3">
                    <span
                      ...
                      [class]="
                        t.activo ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#F3F4F6] text-[#6B7280]'
                      "
                    >
        ...
        >
          <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl my-auto">
            <h2 class="mb-4 text-[18px] font-semibold">
              {{ editingId() ? 'Editar tramitador' : 'Nuevo tramitador' }}
            </h2>
            <div class="space-y-3">
              <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  ><input [(ngModel)]="form.telefono" class="input-field" />
                </div>
                <div>
                  <label class="label-field">Email</label
                  ><input [(ngModel)]="form.email" class="input-field" />
                </div>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                ...
                <div>
                  ...
                  ><input
                    [(ngModel)]="form.comisionValor"
                    type="number"
                    class="input-field"
                    [disabled]="form.comisionTipo === 'NA'"
                  />
                </div>
              </div>
              <div>
...
export class TramitadoresComponent {
  ...
  save(): void {
    ...
    const body = {
      ...
      comisionTipo: this.form.comisionTipo,
      comisionValor: this.form.comisionValor,
    ...
    obs.subscribe({
      next: () => {
        ...
        this.showModal.set(false);
        this.message.set('Tramitador guardado.');
        this.load();
      },
      error: err => {
        this.saving.set(false);
