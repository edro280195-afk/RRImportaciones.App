                  ...
                  <tr class="border-b border-[#F0F2F5] hover:bg-[#F8FAFC] transition-colors">
                    ...
                    <td class="px-4 py-3 text-[#6B717F]">
                      <div class="flex flex-wrap gap-1">
                        @for (a of m.aliases; track a) {
                          <span class="rounded bg-[#F0F2F5] px-1.5 py-0.5 text-[11px]">{{
                            a
                          }}</span>
                        }
                      </div>
                    ...
                    @if (auth.can('CATALOGOS_EDITAR')) {
                      <td class="px-4 py-3">
                        <button
                          (click)="openEdit(m)"
                          class="rounded-lg border border-[#D8DEE8] px-3 py-1.5 text-[12px] font-semibold hover:bg-[#F3F4F6] transition-colors"
                        >
                          Editar
                        </button>
                      </td>
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <input
                  [(ngModel)]="aliasesText"
                  class="input-field"
                  placeholder="ej. TOYTA, TYT, TOYOT"
                />
                <p class="mt-1 text-[11px] text-[#8B93A1]">
                  Nombres alternativos para detectar errores tipográficos en datos importados.
                </p>
              </div>
              <div>
...
export class MarcasComponent {
  ...
  marcasFiltradas(): MarcaDto[] {
    ...
    return this.marcas().filter(
      m => m.nombre.toLowerCase().includes(q) || m.aliases.some(a => a.toLowerCase().includes(q))
    );
  }
