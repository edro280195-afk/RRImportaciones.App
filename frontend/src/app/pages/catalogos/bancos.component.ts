...
@Component({
  ...
  template: `
    <div class="space-y-5">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p class="mt-1 text-[13px] text-[#6B717F]">
            Cuentas disponibles para pagos por transferencia, depósito y referencia bancaria.
          </p>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[700px] text-[13px]">
            ...
            <tbody>
              @if (loading()) {
                <tr>
                  <td colspan="6" class="py-10 text-center text-[13px] text-[#8B93A1]">
                    Cargando...
                  </td>
                </tr>
                ...
                @for (b of filtrados(); track b.id) {
                  <tr class="border-b border-[#F0F2F5] hover:bg-[#F8FAFC] transition-colors">
                    <td class="px-4 py-3">
                      <span
                        class="rounded-md bg-[#F3F4F6] px-2 py-0.5 font-mono text-[11px] font-semibold text-[#4B5162]"
                        >{{ b.identificador }}</span
                      >
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">
                      <span
                        ...
                        [class]="
                          b.activo ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#F3F4F6] text-[#6B7280]'
                        "
      ...
      @if (showModal()) {
        <div
          class="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 backdrop-blur-sm overflow-y-auto"
          (click)="closeModal()"
        >
          <form
            class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl my-auto"
            (click)="$event.stopPropagation()"
            (ngSubmit)="save()"
          >
            <div class="mb-5 flex items-start justify-between gap-4">
              <div>
                <h2 class="text-[18px] font-semibold text-[#0D1017]">
                  {{ editingId() ? 'Editar banco' : 'Nuevo banco' }}
                </h2>
                <p class="text-[12px] text-[#6B717F]">
                  El identificador ayuda a reconocer la cuenta al registrar pagos.
                </p>
              </div>
              <button
                type="button"
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div>
                <label class="label-field">Titular</label
                ><input [(ngModel)]="form.titular" name="titular" class="input-field" />
              </div>
              <div>
                <label class="label-field">Moneda</label
                ><select [(ngModel)]="form.moneda" name="moneda" class="input-field">
              ...
              <div>
                <label class="label-field">Cuenta</label
                ><input [(ngModel)]="form.cuenta" name="cuenta" class="input-field" />
              </div>
              <div>
                <label class="label-field">CLABE</label
                ><input [(ngModel)]="form.clabe" name="clabe" class="input-field" />
              </div>
              <div class="md:col-span-2">
                ...
                ><textarea
                  [(ngModel)]="form.notas"
                  name="notas"
                  class="input-field min-h-[74px]"
                ></textarea>
              </div>
              <label class="flex items-center gap-2 text-[13px] text-[#4B5162]"
                ><input [(ngModel)]="form.activo" name="activo" type="checkbox" /> Activo</label
              >
            </div>
...
export class BancosComponent {
  ...
  filtrados = computed(() => {
    ...
    return this.bancos().filter(b =>
      `${b.identificador} ${b.nombre} ${b.titular ?? ''} ${b.cuenta ?? ''} ${b.clabe ?? ''}`
        .toLowerCase()
        .includes(q)
    );
  });
  ...
  load(): void {
    ...
    this.service.getAll(false).subscribe({
      ...
      error: () => {
        this.loading.set(false);
        this.error.set('No se pudieron cargar los bancos.');
      },
    });
  }
  ...
  save(): void {
    ...
    const body: GuardarBancoRequest = {
      ...this.form,
    ...
    obs.subscribe({
      next: () => {
        ...
        this.showModal.set(false);
        this.message.set('Banco guardado.');
        this.load();
      },
      error: err => {
        this.saving.set(false);
        this.error.set(err?.error?.message || 'Error al guardar banco.');
      },
    });
  }
