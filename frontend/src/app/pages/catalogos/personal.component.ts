                ...
                <tr class="border-b border-[#F0F2F5] hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <td class="px-4 py-3">
                    <span
                      ...
                      [class]="
                        p.activo ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#F3F4F6] text-[#6B7280]'
                      "
                    >
...
export class PersonalComponent {
  ...
  save(): void {
    ...
    obs.subscribe({
      next: () => {
        ...
        this.showModal.set(false);
        this.message.set('Personal guardado.');
        this.load();
      },
      error: err => {
        this.saving.set(false);
  ...
  rolClass(rol: string): string {
    const map: Record<string, string> = {
      CHOFER: 'bg-[#DBEAFE] text-[#1E40AF]',
      ENTREGADOR: 'bg-[#FEF3C7] text-[#92400E]',
      AMBOS: 'bg-[#E0E7FF] text-[#3730A3]',
    };
    return map[rol] ?? 'bg-[#F3F4F6] text-[#374151]';
  }

  private empty() {
    return { nombre: '', rol: 'CHOFER', telefono: '', activo: true };
  }
}
