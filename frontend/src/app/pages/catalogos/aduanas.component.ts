      ...
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p class="mt-1 text-[13px] text-[#6B717F]">
            Catálogo de aduanas registradas. Se administra vía el importador.
          </p>
        </div>
        <div class="relative">
          <input
          ...
          />
          <svg
            class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8B93A1]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[500px] text-[13px]">
            ...
            <tbody>
              @if (loading()) {
                <tr>
                  <td colspan="4" class="py-10 text-center text-[13px] text-[#8B93A1]">
                    Cargando...
                  </td>
                </tr>
                ...
                @for (a of filtradas(); track a.id) {
                  <tr class="border-b border-[#F0F2F5] hover:bg-[#F8FAFC] transition-colors">
                    <td class="px-4 py-3">
                      <span
                        class="rounded-md bg-[#DBEAFE] px-2 py-0.5 font-mono text-[11px] font-semibold text-[#1E40AF]"
                        >{{ a.claveAduana }}</span
                      >
                    </td>
