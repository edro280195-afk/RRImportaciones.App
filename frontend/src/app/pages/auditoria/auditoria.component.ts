  ...
  template: `
    <div class="space-y-5">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p class="mt-1 text-[13px] text-[#6B717F]">
            Historial de cambios importantes: quién hizo qué, cuándo y desde dónde.
          </p>
        </div>
        <div
          class="grid grid-cols-3 gap-2 rounded-2xl border border-[#E4E7EC] bg-white p-2 shadow-sm"
        >
          <div class="px-3 py-2">
      ...
      <div class="card-elevated rounded-2xl p-4">
        <div
          class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-[1fr_1fr_1fr_130px_130px_auto_auto]"
        >
          <div>
            ...
            <input
              ...
              class="input-field"
              placeholder="Trámite, Cotización, Pago..."
              (keyup.enter)="buscar()"
            />
          </div>
          <div>
          ...
          <div class="flex items-end">
            <button (click)="buscar()" class="btn-primary rounded-xl px-4 py-2.5 text-[13px]">
              Buscar
            </button>
          </div>
          <div class="flex items-end">
            <button
              (click)="limpiar()"
              class="rounded-xl border border-[#D8DEE8] px-4 py-2.5 text-[13px] font-semibold text-[#4B5162]"
            >
              Limpiar
            </button>
          </div>
      ...
      <div class="card-elevated overflow-hidden rounded-2xl">
        @if (loading()) {
          ...
          <div
            class="flex flex-col gap-2 border-b border-[#F0F2F5] px-4 py-3 md:flex-row md:items-center md:justify-between"
          >
            <p class="text-[12px] font-semibold text-[#4B5162]">
              {{ resultado()?.total ?? 0 }} registros encontrados
            </p>
            <p class="text-[12px] text-[#8B93A1]">
              Haz clic en una fila para ver el detalle del cambio.
            </p>
          </div>
            ...
            <table class="w-full text-[13px]">
              ...
              <tbody>
                @for (log of resultado()?.items ?? []; track log.id) {
                  <tr
                    class="cursor-pointer border-b border-[#F0F2F5] transition-colors hover:bg-[#F8FAFC]"
                    (click)="toggleDetalle(log)"
                  >
                    <td class="px-4 py-3 font-mono text-[12px] text-[#6B717F]">
                    ...
                    <td class="px-4 py-3">
                      <span
                        ...
                        [class]="accionClass(log.accion)"
                        >{{ accionLabel(log.accion) }}</span
                      >
                    </td>
                    <td class="px-4 py-3 font-semibold text-[#374151]">{{ log.entidad }}</td>
                    <td
                      class="max-w-[220px] truncate px-4 py-3 font-mono text-[11px] text-[#8B93A1]"
                    >
                      {{ log.entidadId || '-' }}
                    </td>
                    <td class="px-4 py-3 font-mono text-[11px] text-[#8B93A1]">
                      {{ log.ipAddress || '-' }}
                    </td>
                  </tr>
                  @if (detalleId() === log.id) {
                    <tr class="bg-[#FAFBFC]">
                      <td colspan="6" class="px-5 py-4">
                        <div
                          class="mb-3 flex flex-wrap items-center gap-2 text-[12px] text-[#6B717F]"
                        >
                          <span class="rounded-full bg-white px-3 py-1 font-semibold">{{
                            log.entidad
                          }}</span>
                            ...
                            @if (rows(log.valoresAnteriores).length) {
                              <div class="space-y-2">
                                @for (row of rows(log.valoresAnteriores); track row.key) {
                                  <div
                                    class="grid grid-cols-[160px_minmax(0,1fr)] gap-3 text-[12px]"
                                  >
                                    <span class="font-semibold text-[#6B717F]">{{
                                      labelKey(row.key)
                                    }}</span>
                        ...
                        <details class="mt-4 rounded-xl border border-[#E4E7EC] bg-white p-3">
                          <summary class="cursor-pointer text-[12px] font-semibold text-[#4B5162]">
                            Ver JSON original
                          </summary>
                          <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
                            <pre
                              class="overflow-x-auto rounded-lg bg-[#FEF2F2] p-3 text-[11px] text-[#374151]"
                              >{{ formatJson(log.valoresAnteriores) }}</pre
                            >
                            <pre
                              class="overflow-x-auto rounded-lg bg-[#F0FDF4] p-3 text-[11px] text-[#374151]"
                              >{{ formatJson(log.valoresNuevos) }}</pre
                            >
                          </div>
                        </details>
                      </td>
                  ...
                  <tr>
                    <td colspan="6" class="px-4 py-10 text-center text-[13px] text-[#8B93A1]">
          ...
          @if ((resultado()?.total ?? 0) > pageSize) {
            <div class="flex items-center justify-between border-t border-[#F0F2F5] px-4 py-3">
              <button
                (click)="prevPage()"
                [disabled]="page() === 1"
                class="rounded-lg border border-[#D8DEE8] px-3 py-1.5 text-[12px] font-semibold disabled:opacity-40"
              >
                Anterior
              </button>
              ...
              <button
                (click)="nextPage()"
                [disabled]="page() * pageSize >= (resultado()?.total ?? 0)"
                class="rounded-lg border border-[#D8DEE8] px-3 py-1.5 text-[12px] font-semibold disabled:opacity-40"
              >
                Siguiente
              </button>
...
export class AuditoriaComponent {
  ...
  rows(json: string | null | undefined): JsonRow[] {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
        return [{ key: 'valor', value: this.stringify(parsed) }];
  ...
  accionClass(accion: string): string {
    ...
    if (accion.includes('DELETE') || accion.includes('BORRAR'))
