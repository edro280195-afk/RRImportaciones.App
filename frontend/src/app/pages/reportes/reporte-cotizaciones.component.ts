    ...
    <div class="space-y-5">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 class="text-[26px] font-semibold leading-none text-[#0D1017]">
        ...
        <div class="flex gap-2">
          <input
            [(ngModel)]="desde"
            type="date"
            class="rounded-xl border border-[#E4E7EC] px-3 py-2 text-[13px]"
          />
          <input
            [(ngModel)]="hasta"
            type="date"
            class="rounded-xl border border-[#E4E7EC] px-3 py-2 text-[13px]"
          />
          <button (click)="load()" class="btn-primary rounded-xl px-4 py-2 text-[13px]">
            Actualizar
          </button>
        </div>
      </div>
      ...
      @if (data) {
        <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
          <div class="metric">
            <span>Emitidas</span><strong>{{ data.totalEmitidas }}</strong>
          </div>
          <div class="metric">
            <span>Aceptadas</span><strong>{{ data.totalAceptadas }}</strong>
          </div>
          <div class="metric">
            <span>Rechazadas</span><strong>{{ data.totalRechazadas }}</strong>
          </div>
          <div class="metric">
            <span>Expiradas</span><strong>{{ data.totalExpiradas }}</strong>
          </div>
          <div class="metric">
            <span>Conversión</span
            ><strong>{{ data.tasaConversionGlobal | number: '1.0-2' }}%</strong>
          </div>
        </div>

        <section class="card-elevated rounded-2xl p-5">
          <div #chart class="mb-6 h-[280px] w-full"></div>
          <h2 class="mb-4 text-[15px] font-semibold text-[#0D1017]">
          ...
          @for (c of data.topClientes; track c.clienteId || c.cliente) {
            <div class="mb-3 grid grid-cols-[180px_minmax(0,1fr)_70px] items-center gap-3">
              ...
              <div class="h-3 overflow-hidden rounded-full bg-[#EEF1F5]">
                <div
                  class="h-full rounded-full bg-[#C61D26]"
      ...
      series: [
        {
          ...
          data: [
            ...
            { value: this.data.totalExpiradas, name: 'Expiradas' },
            {
              value: Math.max(
                0,
                this.data.totalEmitidas -
                  this.data.totalAceptadas -
                  this.data.totalRechazadas -
                  this.data.totalExpiradas
              ),
              name: 'Pendientes',
            },
          ],
