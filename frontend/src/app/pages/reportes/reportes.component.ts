  ...
  template: `
    <div class="space-y-4 pb-10">
      ...
      <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p class="mb-1 text-[11px] font-semibold uppercase tracking-[1.4px] text-[#A4A0A5]">
            Análisis
          </p>
          <h1 class="text-[26px] font-bold leading-none text-[#0D1017] tracking-tight">
            Panel de Reportes
          </h1>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div
            class="flex items-center gap-1.5 rounded-xl border border-[#E4E7EC] bg-white px-3 py-2 shadow-[0_1px_2px_rgba(16,18,23,.04)]"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              class="w-4 h-4 stroke-2 text-[#A4A0A5] shrink-0"
            >
              <path
            ...
            <input
              ...
              type="date"
              class="border-0 bg-transparent text-[13px] text-[#0D1017] outline-none w-[122px]"
            />
        ...
        <div class="space-y-4 animate-in">
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="kpi kpi-accent">
              ...
              <strong class="kpi-val">{{
                fin.cobradoTotal | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
              @if (comparativoActivo() && financieroAnt()) {
                <span
                  class="kpi-delta"
                  [ngClass]="
                    deltaPositive(fin.cobradoTotal, financieroAnt()!.cobradoTotal)
                      ? 'delta-up'
                      : 'delta-down'
                  "
                >
                  {{ deltaStr(fin.cobradoTotal, financieroAnt()!.cobradoTotal)
                  }}<span class="delta-ref">vs. anterior</span>
                </span>
              }
            </div>
            <div class="kpi">
              <span class="kpi-label">Por Cobrar</span>
              <strong class="kpi-val" [style.color]="fin.porCobrarTotal > 0 ? '#D97706' : null">{{
                fin.porCobrarTotal | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
              @if (comparativoActivo() && financieroAnt()) {
                <span
                  class="kpi-delta"
                  [ngClass]="
                    deltaPositive(fin.porCobrarTotal, financieroAnt()!.porCobrarTotal, false)
                      ? 'delta-up'
                      : 'delta-down'
                  "
                >
            ...
            <div class="kpi">
              ...
              <strong
                class="kpi-val"
                [style.color]="fin.margenBruto >= 0 ? '#16A34A' : '#C61D26'"
                >{{ fin.margenBruto | currency: 'MXN' : 'symbol' : '1.0-0' }}</strong
              >
              @if (comparativoActivo() && financieroAnt()) {
                <span
                  class="kpi-delta"
                  [ngClass]="
                    deltaPositive(fin.margenBruto, financieroAnt()!.margenBruto)
                      ? 'delta-up'
                      : 'delta-down'
                  "
                >
            ...
            <div class="kpi">
              ...
              <strong class="kpi-val">{{
                fin.gastosHormigaTotal | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
              @if (comparativoActivo() && financieroAnt()) {
                <span
                  class="kpi-delta"
                  [ngClass]="
                    deltaPositive(
                      fin.gastosHormigaTotal,
                      financieroAnt()!.gastosHormigaTotal,
                      false
                    )
                      ? 'delta-up'
                      : 'delta-down'
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            <div class="kpi kpi-sm">
              ...
              ><strong
                class="kpi-val-sm"
                [style.color]="fin.pagosPendientesVerificacion > 0 ? '#D97706' : null"
                >{{ fin.pagosPendientesVerificacion }}</strong
              >
            </div>
          </div>
              ...
              <div class="overflow-x-auto">
                <table class="rep-table">
                  <thead>
                    <tr>
                      <th>Mes</th>
                      <th class="text-right">Cobrado Verificado</th>
                    </tr>
                  </thead>
                  <tbody>
                    @for (m of fin.evolucionMensual; track m.mesNombre) {
                      <tr>
                        <td>{{ m.mesNombre }}</td>
                        <td class="text-right font-mono-data font-semibold">
                          {{ m.cobradoVerificado | currency: 'MXN' : 'symbol' : '1.0-0' }}
                        </td>
                      </tr>
                    }
                  </tbody>
      ...
      @if (!loading() && activeTab() === 'Pipeline' && pipelineData()) {
        ...
        <div class="space-y-4 animate-in">
          <div class="grid grid-cols-3 gap-3">
            ...
            <div class="kpi">
              <span class="kpi-label">Valor en proceso</span
              ><strong class="kpi-val text-[20px]">{{
                totalPipeline(pipe) | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
            </div>
          </div>
              ...
              <table class="rep-table">
                <thead>
                  <tr>
                    ...
                    <th class="text-right">Días Prom.</th>
                  </tr>
                </thead>
                <tbody>
                  @for (e of pipe.estados; track e.estado) {
                    <tr class="cursor-pointer" (click)="irATramites(e.estado)">
                      <td>
                        ...
                        ><span class="ml-1.5 text-[11px] text-[#A4A0A5]">{{ e.estado }}</span>
                      </td>
                      <td class="text-center font-mono-data font-semibold">{{ e.cantidad }}</td>
                      <td class="text-right font-mono-data">
                        {{ e.montoTotal | currency: 'MXN' : 'symbol' : '1.0-0' }}
                      </td>
                      <td class="text-right font-mono-data">
                        {{ e.diasPromedioEnEstado | number: '1.1-1' }}
                      </td>
                    </tr>
                  }
                </tbody>
                ...
                <tbody>
                  @for (t of prod.tramitadores; track t.tramitadorId) {
                    <tr>
                      ...
                      <td class="text-right font-mono-data">
                        {{ t.montoTotalCobrado | currency: 'MXN' : 'symbol' : '1.0-0' }}
                      </td>
                      <td class="text-right font-mono-data text-[#16A34A] font-semibold">
                        {{ t.montoTotalVerificado | currency: 'MXN' : 'symbol' : '1.0-0' }}
                      </td>
                      <td class="text-right font-mono-data">
                        {{ t.diasPromedioResolucion | number: '1.1-1' }}
                      </td>
                    </tr>
                  }
                </tbody>
        ...
        <div class="space-y-4 animate-in">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div class="kpi kpi-accent">
              ...
              ><strong class="kpi-val">{{
                g.totalPeriodo | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
            </div>
            <div class="kpi">
              <span class="kpi-label">Cargable al Cliente</span
              ><strong class="kpi-val text-[#16A34A]">{{
                g.totalCargableCliente | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
            </div>
            <div class="kpi">
              <span class="kpi-label">Costo Propio</span
              ><strong class="kpi-val">{{
                g.totalCostoPropio | currency: 'MXN' : 'symbol' : '1.0-0'
              }}</strong>
            </div>
          </div>
                ...
                <table class="rep-table">
                  ...
                  <tbody>
                    @for (c of g.porCategoria; track c.categoria) {
                      <tr>
                        <td>{{ c.categoria }}</td>
                        <td class="text-center font-mono-data">{{ c.cantidad }}</td>
                        <td class="text-right font-mono-data font-semibold">
      ...
      @if (!loading() && activeTab() === 'Cotizaciones' && cotizaciones()) {
        ...
        <div class="space-y-4 animate-in">
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div class="kpi kpi-sm kpi-accent col-span-2 sm:col-span-1">
            ...
            <div class="card p-5">
              ...
              <div class="space-y-3">
                @for (c of cot.topClientes.slice(0, 8); track c.cliente) {
                  <div class="flex items-center gap-3">
                    <span
                      class="w-[140px] shrink-0 truncate text-[13px] font-medium text-[#0D1017]"
                      >{{ c.cliente }}</span
                    >
                    <div class="flex-1 h-2.5 overflow-hidden rounded-full bg-[#F1F2F4]">
                      <div
                        class="h-full rounded-full bg-gradient-to-r from-[#C61D26] to-[#E0353E] transition-all duration-700"
                        [style.width.%]="barWidth(c.totalCotizaciones)"
                      ></div>
                    </div>
                    <span class="w-7 text-right font-mono-data text-[12px] text-[#6B717F]">{{
                      c.totalCotizaciones
                    }}</span>
                  </div>
                }
              ...
              @if (cot.tiempoPromedioAceptacionDias > 0) {
                <div class="mt-5 pt-4 border-t border-[#F1F2F4] flex items-center gap-3">
                  <div
                    class="w-9 h-9 rounded-full bg-[#EFF6FF] flex items-center justify-center shrink-0"
                  >
                    <svg
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      class="w-4 h-4 stroke-2 text-[#2563EB]"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                  ...
                  <div>
                    ...
                    <p class="text-[15px] font-semibold text-[#0D1017]">
                      {{ cot.tiempoPromedioAceptacionDias | number: '1.1-1' }} días
                    </p>
                  </div>
                </div>
              }
...
export class ReportesComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      const el = this.chartFinEl()?.nativeElement;
      const d = this.financiero();
      if (el && d) this.renderFinanciero(el, d);
    });
    effect(() => {
      const el = this.chartPipeEl()?.nativeElement;
      const d = this.pipelineData();
      if (el && d) this.renderPipeline(el, d);
    });
    effect(() => {
      const el = this.chartProdEl()?.nativeElement;
      const d = this.productividad();
      if (el && d) this.renderProductividad(el, d);
    });
    effect(() => {
      const el = this.chartGastosEl()?.nativeElement;
      const d = this.gastos();
      if (el && d) this.renderGastos(el, d);
    });
    effect(() => {
      const el = this.chartCotEl()?.nativeElement;
      const d = this.cotizaciones();
      if (el && d) this.renderCotizaciones(el, d);
    });
  }
    ...
    inst.setOption(
      {
        tooltip: {
          ...
          formatter: (p: any[]) => `${p[0].name}<br/><b>${MXN0(p[0].value)}</b>`,
        },
        ...
        xAxis: {
          ...
          axisLabel: {
            ...TEXT_STYLE,
            interval: 0,
            rotate: fin.evolucionMensual.length > 6 ? 30 : 0,
          },
        ...
        series: [
          {
            ...
            itemStyle: {
              color: {
                ...
                y2: 1,
                colorStops: [
                  { offset: 0, color: '#E0353E' },
                  { offset: 1, color: '#C61D26' },
                ],
              },
  ...
  private async renderPipeline(el: HTMLDivElement, pipe: ReportePipelineDto): Promise<void> {
    ...
    inst.setOption(
      {
        ...
        yAxis: {
          ...
          axisLabel: { ...TEXT_STYLE, width: 140, overflow: 'truncate' },
          axisLine: { show: false },
          axisTick: { show: false },
        },
        series: [
          {
            ...
            itemStyle: {
              color: {
                ...
                y2: 0,
                colorStops: [
                  { offset: 0, color: '#C61D26' },
                  { offset: 1, color: '#E0353E' },
                ],
              },
    ...
    inst.on('click', (params: any) => {
      ...
      if (estado) this.irATramites(estado);
    });
  }

  private async renderProductividad(
    el: HTMLDivElement,
    prod: ReporteProductividadDto
  ): Promise<void> {
    ...
    inst.setOption(
      {
        ...
        xAxis: {
          ...
          axisLabel: {
            ...TEXT_STYLE,
            rotate: prod.tramitadores.length > 4 ? 22 : 0,
            width: 90,
            overflow: 'truncate',
          },
          axisLine: { lineStyle: { color: '#E4E7EC' } },
    ...
    inst.setOption(
      {
        tooltip: {
          trigger: 'item',
          formatter: (p: any) => `${p.name}<br/><b>${MXN0(p.value)}</b> (${p.percent}%)`,
        },
        legend: {
          bottom: 0,
          textStyle: TEXT_STYLE,
  ...
  private async renderCotizaciones(
    el: HTMLDivElement,
    cot: ConversionCotizacionesDto
  ): Promise<void> {
  ...
  barWidth(value: number): number {
    const max = Math.max(
      ...(this.cotizaciones()?.topClientes.map(x => x.totalCotizaciones) ?? [1])
    );
