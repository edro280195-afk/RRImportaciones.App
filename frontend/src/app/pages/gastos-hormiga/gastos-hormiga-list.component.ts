  ...
  template: `
    <div style="font-family: var(--font-body);">
      <div class="flex items-start justify-between gap-4 mb-6">
        ...
        <button
          (click)="openCreate()"
          class="px-4 py-2 rounded-xl bg-[#0D1017] text-white text-[13px] font-medium"
        >
          Nuevo gasto
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-5">
        <aside class="card-elevated rounded-2xl p-4 h-fit">
          ...
          <input
            ...
            type="date"
            class="w-full px-3 py-2 rounded-xl border border-[#E4E7EC] text-[13px] mb-2"
            (change)="load()"
          />
          <input
          ...
          />
          <select
            [(ngModel)]="filters.tipoGastoId"
            class="w-full px-3 py-2 rounded-xl border border-[#E4E7EC] text-[13px]"
            (change)="load()"
          >
        ...
        <section>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            ...
            <div class="card-elevated rounded-xl p-4">
              <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Cargable</p>
      ...
      @if (showModal) {
        <div
          class="fixed inset-0 bg-black/35 z-50 flex items-start justify-center px-4 pt-[7vh] overflow-y-auto"
          (click)="showModal = false"
        >
          <form
            class="bg-white rounded-2xl p-6 w-full max-w-[680px] shadow-2xl my-auto"
          ...
          >
            <div class="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                <h3 class="text-[18px] font-semibold text-[#0D1017] mt-1">
                  {{ editingGasto ? 'Editar gasto hormiga' : 'Nuevo gasto hormiga' }}
                </h3>
                <p class="text-[12px] text-[#6B717F] mt-1">
                  {{
                    editingGasto
                      ? 'Los cambios quedaran visibles en auditoria.'
                      : 'Captura el costo, relacionalo y define si se recupera.'
                  }}
                </p>
              </div>
              <button
                type="button"
                (click)="showModal = false"
                class="h-9 w-9 rounded-xl border border-[#E4E7EC] text-[#6B717F] hover:text-[#0D1017]"
              >
                x
              </button>
            </div>
            <div class="rounded-2xl border border-[#EEF0F4] bg-[#F9FAFB] p-4 mb-4">
              <p class="text-[10px] uppercase tracking-[0.8px] font-semibold text-[#9EA3AE]">
                Importe capturado
              </p>
              <div class="flex flex-wrap items-end justify-between gap-2 mt-1">
                <p class="text-[28px] font-semibold text-[#0D1017] font-mono-data">
                  {{ form.monto || 0 | currency: form.moneda }}
                </p>
                <span
                  class="px-3 py-1 rounded-full text-[11px] font-semibold"
                  [class.bg-[#DCFCE7]]="form.seCargaAlCliente"
                  [class.text-[#166534]]="form.seCargaAlCliente"
                  [class.bg-[#FEE2E2]]="!form.seCargaAlCliente"
                  [class.text-[#991B1B]]="!form.seCargaAlCliente"
                >
              ...
              <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162]">
                ...
                >
                  ...
                  @for (v of vehiculos; track v.id) {
                    <option [value]="v.id">{{ v.vinCorto || v.vin }}</option>
                  }
                </select>
              </label>
              <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162]">
                ...
                >
                  ...
                  @for (t of tramites; track t.id) {
                    <option [value]="t.id">
                      ...
                      {{ t.vehiculoMarcaModelo ? '· ' + t.vehiculoMarcaModelo : '' }}
                    </option>
                  }
                </select>
              </label>
            ...
            <label class="flex flex-col gap-1 text-[12px] font-medium text-[#4B5162] mt-3">
              ...
              <input
                ...
                name="concepto"
                placeholder="Ej. caseta, copia, estacionamiento"
                class="w-full px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] font-normal text-[#0D1017]"
              />
            </label>
            <label
              class="mt-4 flex items-center justify-between rounded-2xl border border-[#E4E7EC] px-4 py-3 text-[13px] text-[#4B5162]"
            >
...
export class GastosHormigaListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tramiteService
      .getList({ pageSize: 200, orderBy: 'numero', orderDir: 'desc' })
  ...
  async deleteGasto(gasto: GastoHormigaListDto): Promise<void> {
    const confirmed = await this.notifications.confirm({
      ...
      message:
        'Se ocultara de reportes operativos. La bitacora administrativa conservara el movimiento.',
