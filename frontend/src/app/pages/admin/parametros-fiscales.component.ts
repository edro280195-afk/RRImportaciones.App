...
import {
  ...
  ParametroFiscalDto,
  UpdateParametroFiscalRequest,
} from '../../services/parametros-fiscales.service';
    ...
    <div class="space-y-5">
      <div>
        ...
        <p class="mt-1 text-[13px] text-[#6B717F]">
          Al guardar un cambio se crea una nueva versión con fecha de inicio, conservando el
          histórico.
        </p>
      </div>
          ...
          @for (p of parametros(); track p.id) {
            <div class="card-elevated rounded-2xl p-5">
              <div class="mb-4 flex items-center justify-between">
                ...
                @if (p.activo) {
                  <span
                    class="rounded-full bg-[#DCFCE7] px-2.5 py-0.5 text-[11px] font-semibold text-[#166534]"
                    >Vigente</span
                  >
                } @else {
                  <span
                    class="rounded-full bg-[#F3F4F6] px-2.5 py-0.5 text-[11px] font-semibold text-[#6B7280]"
                    >Histórico</span
                  >
                }
              </div>
              ...
              <div class="mb-4 grid grid-cols-3 gap-3 rounded-xl bg-[#F8FAFC] p-4 text-[13px]">
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">IGI</p>
                  <p class="font-mono font-semibold">
                    {{ p.igi !== null ? (p.igi * 100).toFixed(1) + '%' : '—' }}
                  </p>
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">DTA</p>
                  <p class="font-mono font-semibold">{{ dtaLabel(p) }}</p>
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">IVA</p>
                  <p class="font-mono font-semibold">{{ (p.iva * 100).toFixed(0) + '%' }}</p>
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">
                    PREV fijo
                  </p>
                  <p class="font-mono font-semibold">
                    {{ p.prevFijo > 0 ? '$' + p.prevFijo : '—' }}
                  </p>
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">
                  ...
                  <p class="font-mono font-semibold">{{ p.prvFijo > 0 ? '$' + p.prvFijo : '—' }}</p>
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-[0.8px] text-[#8B93A1]">
                    Desde
                  </p>
                  <p class="font-mono font-semibold text-[11px]">{{ p.vigenteDesde ?? '—' }}</p>
                </div>
              </div>
                  ...
                  <div class="space-y-3 border-t border-[#F0F2F5] pt-4">
                    <div class="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          [(ngModel)]="forms[p.regimen].dtaFijo"
                          type="number"
                          class="input-field"
                          placeholder="ej. 408"
                        />
                      ...
                      <div>
                        ...
                        <input
                          [(ngModel)]="forms[p.regimen].prevFijo"
                          type="number"
                          class="input-field"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          [(ngModel)]="forms[p.regimen].prvFijo"
                          type="number"
                          class="input-field"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        [(ngModel)]="forms[p.regimen].vigenteDesde"
                        type="date"
                        class="input-field"
                      />
                    </div>
                    <div class="flex justify-end gap-2 pt-1">
                      <button
                        (click)="cancelEdit()"
                        class="rounded-xl border border-[#D8DEE8] px-4 py-2 text-[12px] font-semibold"
                      >
                        Cancelar
                      </button>
                      <button
                        (click)="save(p.regimen)"
                        [disabled]="saving()"
                        class="btn-primary rounded-xl px-4 py-2 text-[12px] disabled:opacity-40"
                      >
...
export class ParametrosFiscalesComponent {
  ...
  save(regimen: string): void {
    ...
    this.service.update(regimen, request).subscribe({
      next: () => {
        ...
        this.editingRegimen.set(null);
        this.message.set(
          `Parámetros de ${regimen} actualizados. El histórico anterior fue conservado.`
        );
  ...
  regimenClass(regimen: string): string {
    return regimen === 'POST_2017' ? 'bg-[#DBEAFE] text-[#1E40AF]' : 'bg-[#FEF3C7] text-[#92400E]';
  }
}
