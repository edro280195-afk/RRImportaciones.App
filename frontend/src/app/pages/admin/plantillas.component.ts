...
import {
  GuardarPlantillaMensajeRequest,
  PlantillaMensajeDto,
  ...
  template: `
    <div class="flex flex-col gap-4">
      ...
      <div class="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <p
            style="font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--n-400);margin-bottom:3px;"
          >
            Administración
          </p>
          <h1
            style="font-size:22px;font-weight:700;letter-spacing:-0.4px;color:var(--n-900);margin:0;"
          >
        ...
        <div style="display:flex;align-items:center;gap:10px;">
          @if (message()) {
            <div
              style="display:flex;align-items:center;gap:7px;padding:8px 14px;border-radius:10px;border:1px solid #BBF7D0;background:#F0FDF4;font-size:13px;font-weight:500;color:#166534;"
            >
          ...
          @if (error()) {
            <div
              style="display:flex;align-items:center;gap:7px;padding:8px 14px;border-radius:10px;border:1px solid #FECACA;background:#FEF2F2;font-size:13px;font-weight:500;color:#991B1B;"
            >
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                <circle cx="6.5" cy="6.5" r="5.5" stroke="#991B1B" stroke-width="1.4" />
                <path
      ...
      <div class="mobile-tabbar" aria-label="Secciones de plantillas">
        <button
          type="button"
          [class.mobile-tabbar--active]="mobileTab() === 'plantillas'"
          (click)="mobileTab.set('plantillas')"
        >
          Plantillas
        </button>
        <button
          type="button"
          [class.mobile-tabbar--active]="mobileTab() === 'editar'"
          (click)="mobileTab.set('editar')"
        >
          Editar
        </button>
        <button
          type="button"
          [class.mobile-tabbar--active]="mobileTab() === 'preview'"
        ...
        <button
          type="button"
          [class.mobile-tabbar--active]="mobileTab() === 'variables'"
          (click)="mobileTab.set('variables')"
        >
          Variables
        </button>
      </div>
      ...
      <div class="workspace-shell">
          ...
          <div class="tpl-list-scroll">
            @if (loading()) {
              @for (i of [1, 2, 3]; track i) {
                <div
                  class="shimmer"
                  style="height:52px;border-radius:10px;margin-bottom:4px;"
                ></div>
              }
            } @else {
              @for (p of plantillas(); track p.id) {
                <button
                  ...
                  (click)="select(p)"
                  class="tpl-row"
                  [class.tpl-row--active]="selected()?.id === p.id"
                >
                  <span
                    style="display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:2px;"
                  >
                    <span
                      style="font-size:12px;font-weight:600;color:var(--n-800);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;text-align:left;"
                      >{{ p.codigo }}</span
                    >
                    <span
                      class="ch-badge"
                      [class]="channelFromCode(p.codigo) === 'EMAIL' ? 'ch-email' : 'ch-wa'"
                      >{{ channelFromCode(p.codigo) === 'EMAIL' ? 'EM' : 'WA' }}</span
                    >
                  </span>
                  <span
                    style="display:block;font-size:11px;color:var(--n-500);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:left;"
                    >{{ p.asunto || plainText(p.cuerpo) || 'Sin contenido' }}</span
                  >
                </button>
              } @empty {
                <div style="padding:32px 16px;text-align:center;">
                  <p style="font-size:12px;color:var(--n-400);">Sin plantillas aún.</p>
                </div>
              }
            }
          </div>
        ...
        <main class="col-canvas" [class.mobile-active]="mobileTab() === 'editar'">
          <div class="meta-bar">
            ...
            <div style="display:flex;align-items:center;gap:10px;">
              <div class="ch-toggle">
                <button
                  type="button"
                  [class.ch-toggle--on]="form.channel === 'EMAIL'"
                ...
                <button
                  type="button"
                  [class.ch-toggle--on]="form.channel === 'WHATSAPP'"
              ...
              <label
                style="display:flex;align-items:center;gap:5px;font-size:12px;font-weight:500;color:rgba(30,35,46,.65);cursor:pointer;user-select:none;"
              >
                <input
                  type="checkbox"
                  [(ngModel)]="form.activa"
                  style="accent-color:var(--rr-600);cursor:pointer;"
                />
                Activa
              </label>
            </div>
          ...
          <div class="studio-toolbar">
            ...
            <div class="studio-toolbar-actions">
              @for (s of snippets; track s.label) {
                <button type="button" class="snippet-btn" (click)="insertSnippet(s.text)">
                  {{ s.label }}
                </button>
              }
            </div>
          ...
          <div
            #editorFrame
            class="canvas-card"
            [class.canvas-card--drag]="dragging()"
            (drop)="dropVariable($event, 'message')"
            (dragover)="previewDrop($event)"
          ...
          >
            ...
            @if (form.channel === 'EMAIL') {
              <div class="subject-wrap">
                <textarea
                  #subjectInput
                  class="subject-ghost subject-ghost-area"
                  [(ngModel)]="form.asunto"
                  placeholder="Asunto del correo..."
                  rows="2"
                  (drop)="dropVariable($event, 'subject')"
            ...
            <div
              ...
              [attr.data-placeholder]="
                form.channel === 'EMAIL'
                  ...
                  : 'Escribe el mensaje de WhatsApp. Arrastra variables desde la derecha.'
              "
              (input)="syncMessageFromEditor()"
              (click)="handleEditorClick($event)"
              (keydown)="handleEditorKeydown($event)"
              (keyup)="rememberEditorRange()"
              (mouseup)="rememberEditorRange()"
            ...
            <div class="canvas-footer">
              <div style="display:flex;flex-wrap:wrap;gap:5px;flex:1;min-width:0;">
                @for (tok of usedVariables(); track tok) {
                  <button type="button" class="used-tag" (click)="insertVariable(tok, 'message')">
                    {{ '{' + tok + '}' }}
                  </button>
                } @empty {
                  <span style="font-size:11px;color:var(--n-400);"
                    >Las variables usadas aparecerán aquí.</span
                  >
                }
              </div>
              <span style="font-size:11px;color:var(--n-400);white-space:nowrap;flex-shrink:0;"
          ...
          <div class="actions-bar">
            <div style="display:flex;align-items:center;gap:4px;">
              ...
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                  <path
                    d="M2.5 6.5C2.5 4.29 4.29 2.5 6.5 2.5c1.74 0 3.23 1.05 3.88 2.56M2.5 6.5V3.5M2.5 6.5H5.5"
                    stroke="currentColor"
                    stroke-width="1.5"
                    stroke-linecap="round"
                    stroke-linejoin="round"
              ...
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                  <path
                    d="M10.5 6.5C10.5 4.29 8.71 2.5 6.5 2.5c-1.74 0-3.23 1.05-3.88 2.56M10.5 6.5V3.5M10.5 6.5H7.5"
                    stroke="currentColor"
                    stroke-width="1.5"
                    stroke-linecap="round"
                    stroke-linejoin="round"
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              @if (selected()?.id) {
                <button type="button" (click)="deleteSelected()" class="delete-btn">
                  Eliminar
                </button>
              }
              <button
                ...
                [disabled]="saving()"
                class="btn-primary"
                style="border-radius:10px;padding:8px 22px;font-size:13px;"
              >
        ...
        <aside class="col-preview" [class.mobile-active]="mobileTab() === 'preview'">
          <div class="col-header" style="border-bottom:1px solid var(--border);">
            ...
            <div class="preview-toggle">
              <button
                type="button"
                [class.preview-toggle--on]="previewMode() === 'mobile'"
              ...
              <button
                type="button"
                [class.preview-toggle--on]="previewMode() === 'desktop'"
              ...
              @if (form.channel === 'WHATSAPP') {
                <div class="phone-shell">
                  <div class="phone-inner">
                    ...
                    <div class="wa-header">
                      <div
                        style="width:26px;height:26px;border-radius:50%;background:rgba(255,255,255,.2);flex-shrink:0;"
                      ></div>
                      <div>
                        <p
                          style="color:white;font-size:10px;font-weight:600;margin:0;line-height:1.2;"
                        >
                          R&amp;R Importaciones
                        </p>
                        <p style="color:rgba(255,255,255,.65);font-size:8.5px;margin:0;">
                          en línea
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="wa-input-bar">
                      ...
                      <div class="wa-send-btn">
                        <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                          <path d="M1.5 1.5l8 4-8 4V7l5.5-1.5L1.5 4V1.5z" fill="white" />
                        </svg>
                      </div>
                    </div>
                ...
                <div class="phone-shell">
                  <div class="phone-inner">
                    ...
                    <div class="mail-mobile-header">
                      <p
                        style="font-size:11px;font-weight:700;color:#1a1a1a;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                      >
                        {{ renderText(form.asunto || 'Sin asunto') }}
                      </p>
                      <p style="font-size:9.5px;color:#888;margin:3px 0 0;">
                        R&amp;R Importaciones
                      </p>
                    </div>
                    <div class="mail-mobile-body">
                      <p class="preview-body">{{ previewText() }}</p>
                    </div>
                    <div class="phone-home-bar"></div>
                  </div>
              ...
              @if (form.channel === 'EMAIL') {
                <div class="mail-window">
                  <div class="mail-chrome">
                    <span class="dot dot-red"></span><span class="dot dot-yellow"></span
                    ><span class="dot dot-green"></span>
                    <span
                      style="flex:1;text-align:center;font-size:10.5px;color:#666;font-weight:500;"
                      >Mail</span
                    >
                  </div>
                  <div class="mail-meta">
                    ...
                    <div style="flex:1;min-width:0;">
                      <p style="font-size:11.5px;font-weight:600;color:#1a1a1a;margin:0;">
                        R&amp;R Importaciones
                      </p>
                      <p style="font-size:9.5px;color:#888;margin:1px 0 0;">
                        contacto&#64;rrimportaciones.com
                      </p>
                    </div>
                  </div>
                ...
                <div class="wa-web-shell">
                  <div class="wa-web-sidebar">
                    <div style="padding:10px 8px;border-bottom:1px solid rgba(255,255,255,.06);">
                      <div
                        style="width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,.2);"
                      ></div>
                    </div>
                    <div style="padding:8px 6px;">
                      <div
                        style="background:rgba(255,255,255,.10);border-radius:6px;padding:7px 8px;"
                      >
                        <p style="font-size:9.5px;font-weight:600;color:white;margin:0 0 1px;">
                          R&amp;R Import.
                        </p>
                        <p
                          style="font-size:8.5px;color:rgba(255,255,255,.45);margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                        >
                          {{ (form.message || '...').substring(0, 24) }}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div class="wa-web-main">
                    <div class="wa-web-header">
                      <div
                        style="width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,.25);flex-shrink:0;"
                      ></div>
                      <div>
                        <p style="font-size:10.5px;font-weight:600;color:white;margin:0;">
                          R&amp;R Importaciones
                        </p>
        ...
        <aside class="col-vars" [class.mobile-active]="mobileTab() === 'variables'">
          ...
          @if (selectedVariable(); as key) {
            <div class="selected-var-box">
              ...
              <p style="font-size:12px;font-weight:600;color:var(--n-800);margin-bottom:2px;">
                {{ variableLabel(key) }}
              </p>
              <p style="font-size:10px;color:var(--n-500);font-family:var(--font-mono);">
                {{ '{' + key + '}' }}
              </p>
              <button type="button" (click)="removeSelectedChip()" class="btn-quitar">
                Quitar
              </button>
            </div>
          }
                ...
                @for (v of variablesByCategory(category); track v.key) {
                  ...
                  >
                    ...
                    <div style="flex:1;min-width:0;text-align:left;">
                      <span
                        style="display:block;font-size:11.5px;font-weight:500;color:var(--n-800);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                        >{{ v.label }}</span
                      >
                      <span
                        style="display:block;font-size:10px;color:var(--n-400);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                        >{{ v.sample }}</span
                      >
                    </div>
                  </button>
  ...
  styles: [
    `
      ...
      .status-toast {
        ...
        font-size: 13px;
        font-weight: 700;
        box-shadow: 0 18px 38px rgba(13, 16, 23, 0.18);
      ...
      .tpl-row--active::before {
        ...
        background: var(--rr-600);
      }
      .tpl-row--active:hover:not(:disabled) {
        box-shadow: var(--shadow-sm);
      }
      .ch-badge {
        font-size: 9px;
      ...
      .btn-nueva:hover:not(:disabled) {
        background: var(--rr-700);
        transform: none;
        box-shadow: none;
      }
      .btn-nueva:active:not(:disabled) {
        transform: none;
      }
      ...
      .code-ghost:focus {
        border-color: var(--rr-600);
        box-shadow: 0 0 0 3px rgba(198, 29, 38, 0.1);
      }
      .ch-toggle {
        display: flex;
      ...
      .ch-toggle button:hover:not(:disabled) {
        color: rgba(30, 35, 46, 0.8);
        transform: none;
        box-shadow: none;
      }
      .ch-toggle button:active:not(:disabled) {
        transform: none;
      }
      .ch-toggle button.ch-toggle--on {
        background: white;
        color: var(--n-900);
        font-weight: 600;
      ...
      .studio-toolbar {
        ...
        border-radius: 12px;
        box-shadow: 0 10px 24px rgba(13, 16, 23, 0.08);
      ...
      .canvas-card {
        ...
        border-radius: 16px;
        box-shadow:
          0 8px 32px rgba(13, 16, 23, 0.1),
          0 2px 8px rgba(13, 16, 23, 0.06),
          0 0 0 1px rgba(13, 16, 23, 0.04);
        display: flex;
        flex-direction: column;
        flex: 1;
        min-height: 420px;
      ...
      .canvas-card--drag {
        box-shadow:
          0 0 0 2px var(--rr-600),
          0 8px 32px rgba(13, 16, 23, 0.12);
      }
      ...
      .drop-cursor {
        ...
        border-radius: 2px;
        background: var(--rr-600);
        box-shadow: 0 0 0 3px rgba(198, 29, 38, 0.14);
        pointer-events: none;
        z-index: 10;
      ...
      .subject-ghost {
        ...
        caret-color: var(--rr-600);
        letter-spacing: -0.3px;
      }
      .subject-ghost::placeholder {
        color: var(--n-300);
        font-weight: 400;
      }
      .subject-wrap {
      ...
      .template-composer--email {
        padding: 20px 44px 32px;
      }
      .template-composer--wa {
        padding: 32px 44px;
      }
      .template-composer:empty::before {
      ...
      .used-tag:hover:not(:disabled) {
        background: var(--rr-50);
        border-color: rgba(198, 29, 38, 0.25);
        color: var(--rr-700);
        transform: none;
        box-shadow: none;
      }
      .used-tag:active:not(:disabled) {
        transform: none;
      }
      ...
      .snippet-btn:active:not(:disabled) {
        transform: none;
      }
      .delete-btn {
        font-size: 12px;
      ...
      .preview-toggle button:active:not(:disabled) {
        transform: none;
      }
      .preview-toggle button.preview-toggle--on {
        background: white;
        color: var(--n-800);
        box-shadow: var(--shadow-xs);
      }
      ...
      .var-chip:hover {
        border-color: rgba(198, 29, 38, 0.22);
        box-shadow: var(--shadow-sm);
      }
      ...
      .var-chip:active:not(:disabled) {
        ...
        box-shadow: var(--shadow-lg);
        border-color: rgba(198, 29, 38, 0.4);
      }
      .var-chip-dots {
        color: var(--n-300);
        opacity: 0;
      ...
      .selected-var-box {
        ...
        padding: 10px 12px;
        background: #fff7f7;
        border: 1px solid rgba(198, 29, 38, 0.14);
        border-radius: 10px;
        flex-shrink: 0;
      }
      .selected-var-title {
      ...
      .btn-quitar:active:not(:disabled) {
        transform: none;
      }
      ...
      .template-chip.is-selected {
        box-shadow: 0 0 0 3px rgba(198, 29, 38, 0.18);
        outline: 1.5px solid rgba(198, 29, 38, 0.5);
      }
      .template-chip-dot {
        width: 6px;
        height: 6px;
      ...
      .template-chip:hover .template-chip__remove {
        opacity: 1;
      }
      .template-chip__remove:hover:not(:disabled) {
        background: rgba(0, 0, 0, 0.18);
        transform: none;
        box-shadow: none;
      }
      .template-chip__remove:active:not(:disabled) {
        transform: none;
      }

      @media (max-width: 1240px) {
        .workspace-shell {
          grid-template-columns: 150px 170px minmax(300px, 1fr) 180px;
        }
        .col-canvas {
          padding: 16px;
        }
      ...
      @media (max-width: 1100px) {
        ...
        .mobile-tabbar button:active:not(:disabled) {
          transform: none;
          box-shadow: none;
        }
        .mobile-tabbar button.mobile-tabbar--active {
          background: var(--rr-600);
          color: white;
          box-shadow: 0 10px 20px rgba(198, 29, 38, 0.22);
        }
        ...
        .col-list.mobile-active,
        .col-canvas.mobile-active,
        .col-preview.mobile-active,
        .col-vars.mobile-active {
          display: flex;
        }
...
export class PlantillasComponent implements AfterViewInit {
  ...
  readonly variableCategories: VariableCategory[] = [
    'Cliente',
    'Vehiculo',
    'Cotizacion',
    'Sistema',
  ...
  readonly variableOptions: VariableOption[] = [
    ...
    {
      key: 'valor_aduana_usd',
      label: 'Valor aduana',
      sample: '$18,500.00 USD',
      category: 'Cotizacion',
    },
    {
      key: 'impuestos_total',
      label: 'Impuestos',
      sample: '$74,230.00 MXN',
      category: 'Cotizacion',
    },
    ...
    { key: 'fecha_expiracion', label: 'Vigencia', sample: '21/05/2026', category: 'Cotizacion' },
    {
      key: 'url_pdf',
      label: 'PDF',
      sample: 'https://rrimportaciones.com/cotizacion.pdf',
      category: 'Cotizacion',
    },
    {
      key: 'mensaje_personalizado',
      label: 'Mensaje extra',
      sample: 'Quedo atento a cualquier duda.',
      category: 'Sistema',
    },
    {
      key: 'fechaActual',
      label: 'Fecha actual',
      sample: this.currentDateSample(),
      category: 'Sistema',
    },
  ];
  ...
  private readonly catColors: Record<
    string,
    { bg: string; border: string; dot: string; color: string }
  > = {
  ...
  categoryDot(cat: VariableCategory): string {
    return this.catColors[cat]?.dot ?? '#888';
  }
  ...
  ngAfterViewInit(): void {
    this.renderEditorFromMessage();
  }

  variableLabel(key: string): string {
    return this.variableOptions.find(v => v.key === key)?.label ?? key;
  }
  ...
  load(): void {
    ...
    this.service.getAll().subscribe({
      next: items => {
        ...
        if (!this.selected() && items.length > 0) this.select(items[0]);
      },
      error: () => {
        this.loading.set(false);
  ...
  select(p: PlantillaMensajeDto): void {
    ...
    this.form = {
      codigo: p.codigo,
      asunto: this.removeDeprecatedYearVariable(p.asunto),
      message: this.toEditableMessage(p.cuerpo),
      activa: p.activa,
      channel: this.channelFromCode(p.codigo),
    };
  ...
  setChannel(ch: TemplateChannel): void {
    ...
    if (!this.form.codigo.trim())
      this.form.codigo = ch === 'EMAIL' ? 'COTIZACION_EMAIL' : 'COTIZACION_WHATSAPP';
    if (ch === 'WHATSAPP') this.form.asunto = null;
  }

  variablesByCategory(cat: VariableCategory): VariableOption[] {
    return this.variableOptions.filter(v => v.category === cat);
  }
  ...
  dropVariable(e: DragEvent, target: 'message' | 'subject'): void {
    e.preventDefault();
    const key =
      e.dataTransfer?.getData('application/x-rr-variable') || e.dataTransfer?.getData('text/plain');
    this.clearDropState();
    if (!key) return;
  ...
  insertVariable(key: string, target: 'message' | 'subject' = 'message'): void {
    if (target === 'subject') {
      this.form.asunto = this.insertAtCursor(
        this.form.asunto ?? '',
        `{${key}}`,
  ...
  save(): void {
    if (!this.form.codigo.trim()) {
      this.error.set('El código interno es obligatorio.');
      return;
    }
    ...
    const req: GuardarPlantillaMensajeRequest = {
      ...
      cuerpo: this.technicalOutput(),
      variablesDisponibles: this.variablesJson(),
    ...
    (cur?.id ? this.service.actualizar(cur.id, req) : this.service.crear(req)).subscribe({
      next: saved => {
        ...
        this.select(saved);
        this.message.set('Plantilla actualizada y guardada.');
        this.load();
      },
      error: err => {
        this.saving.set(false);
  ...
  async deleteSelected(): Promise<void> {
    ...
    this.service.eliminar(cur.id).subscribe({
      next: () => {
        this.newTemplate();
        this.load();
        this.message.set('Plantilla eliminada.');
      },
  ...
  handleEditorClick(e: MouseEvent): void {
    ...
    if (rm) {
      e.preventDefault();
      this.removeChipElement(rm.closest('.template-chip') as HTMLElement | null);
      return;
    }
  ...
  handleEditorKeydown(e: KeyboardEvent): void {
    ...
    if ((e.key === 'Backspace' || e.key === 'Delete') && this.selectedVariable()) {
      e.preventDefault();
  ...
  insertSnippet(text: string): void {
    this.insertTextIntoEditor(text);
    this.renderEditorFromMessage();
  }
  undo(): void {
    const p = this.history.pop();
    if (p === undefined) return;
    this.future.push(this.form.message);
    this.form.message = p;
    this.renderEditorFromMessage();
  }
  redo(): void {
    ...
    this.history.push(this.form.message);
    this.form.message = n;
    this.renderEditorFromMessage();
  }
  ...
  pastePlainText(e: ClipboardEvent): void {
    e.preventDefault();
    this.insertTextIntoEditor(e.clipboardData?.getData('text/plain') ?? '');
  }

  previewText(): string {
    return this.renderText(this.form.message || 'Escribe un mensaje para ver la vista previa.');
  }
  previewHtml(): string {
    return this.buildEmailHtml(this.previewText());
  }
  technicalOutput(): string {
    return this.form.channel === 'EMAIL'
      ? this.buildEmailHtml(this.form.message)
      : this.form.message.trim();
  }
  variablesJson(): string {
    return JSON.stringify(this.detectVariables(), null, 2);
  }
  renderText(value: string): string {
    return this.variableOptions.reduce((t, v) => t.replaceAll(`{${v.key}}`, v.sample), value);
  }
  plainText(value: string): string {
    return this.toEditableMessage(value).replace(/\s+/g, ' ').trim();
  }
  channelFromCode(code: string): TemplateChannel {
    return code.toUpperCase().includes('WHATSAPP') ? 'WHATSAPP' : 'EMAIL';
  }
  channelClass(ch: TemplateChannel): string {
    return ch === 'EMAIL' ? 'bg-[#DBEAFE] text-[#1E40AF]' : 'bg-[#DCFCE7] text-[#166534]';
  }
  ...
  private insertAtCursor(
    ...
    const s = inp.selectionStart ?? cur.length,
      e = inp.selectionEnd ?? cur.length;
    const pre = cur.slice(0, s),
      suf = cur.slice(e);
    const nb = pre.length > 0 && !/[\s({[]$/.test(pre),
      na = suf.length > 0 && !/^[\s,.;:!?)}\]]/.test(suf);
    window.setTimeout(() => {
      inp.focus();
      inp.setSelectionRange(
        pre.length + (nb ? 1 : 0) + tok.length,
        pre.length + (nb ? 1 : 0) + tok.length
      );
    });
  ...
  private renderEditorFromMessage(): void {
    ...
    for (const m of this.form.message.matchAll(rx)) {
      if (m.index > last)
        ed.append(document.createTextNode(this.form.message.slice(last, m.index)));
      ed.append(this.createTokenChip(m[1]));
      last = m.index + m[0].length;
    }
    if (last < this.form.message.length)
      ed.append(document.createTextNode(this.form.message.slice(last)));
  }

  private insertTokenIntoEditor(key: string): void {
    const ed = this.messageEditor?.nativeElement;
    if (!ed) {
      this.form.message = this.form.message ? `${this.form.message} {${key}}` : `{${key}}`;
      return;
    }
    this.pushHistory();
    ed.focus();
    ...
    const chip = this.createTokenChip(key),
  ...
  private updateDropMarker(): void {
    const frame = this.editorFrame?.nativeElement,
      range = this.validEditorRange();
    if (!frame || !range) return;
    const rect = range.getBoundingClientRect(),
      fr = frame.getBoundingClientRect();
    if (rect.width === 0 && rect.height === 0) {
      this.dropMarker.set({ left: 44, top: 44, height: 24 });
      return;
    }
    this.dropMarker.set({
      left: Math.max(10, rect.left - fr.left),
      top: Math.max(10, rect.top - fr.top),
      height: Math.max(22, rect.height),
    });
  }

  private selectChip(chip: HTMLElement | null): void {
    this.messageEditor?.nativeElement
      ?.querySelectorAll('.template-chip.is-selected')
    ...
    this.selectedVariable.set(chip.dataset['token'] ?? null);
  }
  ...
  private createTokenChip(key: string): HTMLSpanElement {
    ...
    dot.className = 'template-chip-dot';
    ...
    rm.type = 'button';
    rm.className = 'template-chip__remove';
  ...
  private extractEditorText(root: Node): string {
    ...
    root.childNodes.forEach(node => {
      ...
      if (node instanceof HTMLElement) {
        ...
        const bl = out.length;
        out += this.extractEditorText(node);
        if (
          (node instanceof HTMLDivElement || node instanceof HTMLParagraphElement) &&
          out.length > bl &&
          !out.endsWith('\n')
        )
          out += '\n';
  ...
  private validEditorRange(): Range | null {
    const ed = this.messageEditor?.nativeElement;
    if (!ed || !this.lastEditorRange || !ed.contains(this.lastEditorRange.commonAncestorContainer))
      return null;
  ...
  private applySelection(r: Range): void {
    ...
    sel.removeAllRanges();
    sel.addRange(r);
    this.lastEditorRange = r.cloneRange();
  }
  ...
  private detectVariables(): string[] {
    ...
    if (this.form.asunto)
      for (const m of this.form.asunto.matchAll(/\{([a-zA-Z0-9_]+)\}/g)) used.add(m[1]);
