...
<div style="font-family: var(--font-body); min-height: 100vh;">
  ...
  <div class="mb-6 flex items-start justify-between gap-4 flex-wrap">
    <div>
      <p class="text-[11px] font-semibold uppercase tracking-[1px] text-[#9EA3AE] mb-1">Admin · Catálogos</p>
      <h1 class="text-[26px] font-semibold text-[#0D1017] tracking-[-0.6px]">Catálogo de Precios SAT</h1>
      <p class="text-[13px] text-[#6B7280] mt-0.5">Anexo 2 — precios estimados de importación por modelo y antigüedad</p>
    </div>
  ...
  <div class="filter-bar mb-4">
    ...
    <div class="filter-toggle-wrap">
      <label class="filter-toggle" [class.active]="filterGenerico === true" (click)="toggleGenerico(true)">Solo genéricas</label>
      <label class="filter-toggle" [class.active]="filterGenerico === false" (click)="toggleGenerico(false)">Solo específicas</label>
      @if (filterGenerico !== undefined) {
        <button class="filter-clear-btn" (click)="filterGenerico = undefined; loadPage(1)" title="Quitar filtro">
          <svg viewBox="0 0 16 16" fill="currentColor" width="14" height="14">
            <path d="M4.293 4.293a1 1 0 011.414 0L8 6.586l2.293-2.293a1 1 0 111.414 1.414L9.414 8l2.293 2.293a1 1 0 01-1.414 1.414L8 9.414l-2.293 2.293a1 1 0 01-1.414-1.414L6.586 8 4.293 5.707a1 1 0 010-1.414z"/>
          </svg>
        </button>
        ...
        @if (loading()) {
            ...
            <td colspan="8">
              <div class="empty-state">
                <svg width="40" height="40" viewBox="0 0 40 40" fill="none" class="empty-icon">
                  <rect x="4" y="8" width="32" height="26" rx="4" stroke="#D1D5DB" stroke-width="1.5"/>
                  <path d="M4 14h32" stroke="#D1D5DB" stroke-width="1.5"/>
                  <path d="M13 20h4M13 25h10" stroke="#D1D5DB" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
                <p class="empty-title">Sin resultados</p>
                <p class="empty-sub">Cambia los filtros o importa datos desde el <a routerLink="/admin/importador" class="link">Importador SAT</a>.</p>
              </div>
            </td>
          </tr>
        } @else {
          @for (item of items(); track item.id) {
            ...
            >
              <!-- Expand chevron -->
              <td class="td-chevron">
                <svg class="chevron" [class.rotated]="expandedId() === item.id"
              ...
              <!-- Categoría -->
              <td class="td-cat">
                <span class="cat-pill cat-pill--{{ item.categoria }}">{{ item.categoria }}</span>
              </td>
                ...
                @if (item.precioMinUsd !== null) {
                  <span class="precio-range">
                    @if (item.precioMinUsd === item.precioMaxUsd) {
                      ...
                      ${"$"}{{ item.precioMinUsd | number:'1.0-0' }} – ${"$"}{{ item.precioMaxUsd | number:'1.0-0' }}
                    }
                  </span>
                } @else {
              ...
              <td class="td-actions" (click)="$event.stopPropagation()">
                ...
                >
                  <svg viewBox="0 0 16 16" fill="none" width="14" height="14">
                    <path d="M11.5 2.5a1.5 1.5 0 012.121 2.121L5.5 12.743l-2.828.707.707-2.828L11.5 2.5z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
                  </svg>
                </button>
                <button
                ...
                >
                  @if (deletingId() === item.id) {
                    <svg viewBox="0 0 16 16" fill="none" width="14" height="14">
                      <path d="M3 4h10M6 4V3h4v1M5 4l.5 9h5L11 4" stroke="#C61D26" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  } @else {
                    <svg viewBox="0 0 16 16" fill="none" width="14" height="14">
                ...
                @if (deletingId() === item.id) {
                  <div class="popconfirm">
                    ...
                    <div class="popconfirm-actions">
                      <button class="popconfirm-cancel" (click)="deletingId.set(null)">Cancelar</button>
                      <button class="popconfirm-confirm" (click)="executeDelete(item.id)" [disabled]="deleteLoading()">
                        @if (deleteLoading()) { Eliminando… } @else { Eliminar }
                      </button>
                    </div>
                  </div>
                }
                      ...
                      <div class="detail-grid">
                        <div class="detail-meta">
                          <div class="detail-meta-row">
                            ...
                            <span class="detail-val"><code>{{ detail()!.fraccion }}</code> · {{ detail()!.fraccionDescripcion }}</span>
                          </div>
                            ...
                            <thead>
                              <tr>
                                @for (p of detail()!.precios; track p.id) {
                                  <th>{{ p.antiguedadAnios }} año{{ p.antiguedadAnios !== 1 ? 's' : '' }}</th>
                                }
                              </tr>
                            </thead>
                            <tbody>
...
@if (editPanelOpen()) {
  ...
  <div class="edit-panel" [class.open]="editPanelOpen()">
    ...
    <div class="panel-header">
      <div>
        <p class="text-[11px] font-semibold uppercase tracking-[0.8px] text-[#9EA3AE]">Catálogo SAT</p>
        <h2 class="text-[17px] font-semibold text-[#0D1017] mt-0.5">{{ panelMode() === 'create' ? 'Nueva entrada' : 'Editar entrada' }}</h2>
      </div>
          ...
          <div class="panel-field-row">
            <div class="panel-field">
              ...
              @if (panelMode() === 'create') {
                ...
                <div class="panel-readonly"><code>{{ editForm.fraccion }}</code></div>
              }
            </div>
            <div class="panel-field">
              <label class="panel-lbl">Hoja PDF origen</label>
              <input type="text" class="panel-input" [(ngModel)]="editForm.hojaOrigen" placeholder="ej. Hoja 3">
            </div>
          </div>
        </div>
        ...
        <div class="panel-section">
          ...
          <div class="panel-field-row">
            <div class="panel-field">
              ...
              <input type="text" class="panel-input" [(ngModel)]="editForm.marcaTexto" placeholder="ej. CHEVROLET">
            </div>
            <div class="panel-field">
              <label class="panel-lbl">Modelo</label>
              <input type="text" class="panel-input" [(ngModel)]="editForm.modelo" placeholder="ej. CRUZE">
            </div>
          </div>
          <div class="panel-field-row">
            <div class="panel-field">
            ...
            <div class="panel-field">
              ...
              <input type="text" class="panel-input" [(ngModel)]="editForm.inciso" placeholder="ej. A, B…">
            </div>
          </div>
          ...
          <div class="panel-field">
            ...
            <p class="panel-hint">
              @if (editForm.esGenerico) {
                Precio base sin marca o modelo concreto. Se usa como último recurso en la cotización.
              } @else {
        ...
        <div class="panel-section">
          ...
          @if (editForm.precios.length > 0) {
            <div class="price-edit-grid">
              @for (p of editForm.precios; track panelMode() === 'create' ? p.antiguedadAnios : p.id) {
...
.stat-chip--muted { background: #F9FAFB; }
.stat-num {
  font-size: 14px;
  font-weight: 600;
  color: #0D1017;
...
.filter-search:focus {
  border-color: #C61D26;
  box-shadow: 0 0 0 3px rgba(198,29,38,.08);
  background: #fff;
}
.filter-clear-btn {
  position: absolute;
...
.filter-clear-btn:hover { color: #6B7280; }
.filter-select {
  padding: 7px 10px;
  border: 1px solid #E4E7EC;
  border-radius: 9px;
...
.filter-select:focus { border-color: #C61D26; }
.filter-toggle-wrap {
  display: flex;
  align-items: center;
  gap: 4px;
...
.catalog-row.expanded { background: #FDF9F9; }
.catalog-row td {
  padding: 10px 14px;
  vertical-align: middle;
}
.td-chevron { padding: 10px 6px 10px 12px !important; width: 36px; }
.chevron {
  color: #9EA3AE;
...
.td-marca { color: #374151; font-weight: 500; }
.td-modelo {}
.modelo-text { color: #0D1017; font-weight: 500; }
.inciso-pill {
  display: inline-block;
  margin-left: 5px;
  font-size: 10px;
...
.cat-pill--automovil { background: #EFF6FF; color: #1D4ED8; }
.cat-pill--camioneta { background: #F0FDF4; color: #166534; }
.cat-pill--camion    { background: #FEF9EC; color: #854D0E; }
.cat-pill--pickup    { background: #FDF4FF; color: #6B21A8; }
.cat-pill--tractor   { background: #FFF7ED; color: #9A3412; }
.cat-pill--hibrido   { background: #F0FDF4; color: #166534; }
.cat-pill--electrico { background: #F0FDFA; color: #0F766E; }
.cat-pill--suv       { background: #EFF6FF; color: #1E40AF; }
/* Precio */
...
/* Años pills */
.td-anios { text-align: center; }
.anios-pills { display: flex; gap: 2px; flex-wrap: wrap; justify-content: center; max-width: 180px; }
.anio-pill {
  font-size: 10px;
  font-weight: 500;
  padding: 1px 4px;
...
.action-btn--edit:hover { color: #1D4ED8; }
.action-btn--delete:hover { color: #C61D26; }
.action-btn--delete.confirming { color: #C61D26; background: #FEF2F2; }
/* Popconfirm */
.popconfirm {
  position: absolute;
  right: 0;
...
.popconfirm-actions { display: flex; gap: 6px; justify-content: flex-end; }
.popconfirm-cancel {
  padding: 5px 10px;
  font-size: 12px;
  border: 1px solid #E4E7EC;
...
.popconfirm-confirm:disabled { opacity: .6; cursor: not-allowed; }
...
.detail-meta-row { display: flex; gap: 12px; font-size: 12px; }
.detail-lbl { min-width: 90px; color: #9EA3AE; font-weight: 500; }
...
.empty-sub { font-size: 13px; color: #9EA3AE; text-align: center; max-width: 280px; }
.link { color: #C61D26; text-decoration: underline; }
...
.page-indicator { font-size: 12px; font-weight: 500; padding: 0 4px; }

/* ── Edit panel ──────────────────────────────────────────────────────────── */
.panel-backdrop {
  position: fixed;
  inset: 0;
  background: rgba(13,16,23,.3);
...
.price-edit-cell { display: flex; flex-direction: column; gap: 4px; }
.price-edit-lbl {
  font-size: 10.5px;
  font-weight: 600;
  color: #6B7280;
...
.price-edit-input[type=number] { -moz-appearance: textfield; }
...
.panel-cancel:hover { background: #F3F4F6; }
.panel-save {
  display: flex;
  align-items: center;
  gap: 6px;
...
.panel-save:hover:not(:disabled) { background: #1E2330; }
.panel-save:disabled { opacity: .6; cursor: not-allowed; }
...
.create-btn:hover { background: #A31820; box-shadow: 0 2px 8px rgba(198,29,38,.25); }

/* ── Estilos Responsive ───────────────────────────────────────────────────── */
@media (max-width: 1024px) {
  /* Tabla: ocultar columnas no esenciales */
  .catalog-table th:nth-child(3),  /* Marca */
  .catalog-table td.td-marca { display: none; }
...
@media (max-width: 768px) {
  ...
  .mb-6.flex.items-start { flex-direction: column; }

  /* Filter bar: apilar todos los elementos */
  .filter-bar { flex-direction: column; align-items: stretch; }
  .filter-search-wrap { min-width: unset; }
  .filter-toggle-wrap { justify-content: center; }
  .filter-chips { justify-content: center; }

  /* Tabla: convertir a cards */
  .table-wrap { border-radius: 10px; overflow: visible; }
  .catalog-table, .catalog-table thead { display: none; }
  .catalog-table tbody, .catalog-table tr, .catalog-table td { display: block; }
  ...
  .catalog-row:hover { background: #FAFAFA; }
  .catalog-row td { padding: 4px 0 !important; }
  .td-chevron { display: none !important; }
  .td-fraccion { margin-bottom: 6px; }
  .td-fraccion .fraccion-badge { font-size: 12px; }
  .td-marca { display: block !important; color: #6B7280; font-size: 12px; }
  .td-marca::before { content: "Marca: "; font-weight: 600; color: #9EA3AE; }
  .td-modelo { font-size: 14px; font-weight: 600; }
  .td-cat { margin: 4px 0; }
  .td-precio { text-align: left; }
  .td-anios { text-align: left; }
  .anios-pills { justify-content: flex-start; }
  .td-actions { position: static !important; margin-top: 8px; }

  /* Expanded row en mobile */
  .expanded-body { padding: 12px 14px !important; }
  .detail-meta-row { flex-direction: column; gap: 2px !important; }
  .detail-lbl { min-width: unset !important; }

  /* Panel: full-width en mobile */
  .edit-panel { width: 100vw !important; max-width: 100vw !important; }

  /* Panel grid: single column */
  .panel-field-row { grid-template-columns: 1fr !important; }

  /* Precios edit grid: 3 columnas en mobile */
  .price-edit-grid { grid-template-columns: repeat(3, 1fr) !important; }
...
@media (max-width: 480px) {
  ...
  .price-edit-grid { grid-template-columns: repeat(2, 1fr) !important; }

  /* Skeleton rows también responsive */
  .skeleton-row td { display: block; padding: 8px 14px; }
...
export class CatalogoPreciosComponent implements OnInit {
  ...
  editForm: (CatalogoPrecioDetailDto & { precios: { id: string; antiguedadAnios: number; precioUsd: number }[] }) | null = null;
  ...
  saveEdit(): void {
    ...
    if (this.panelMode() === 'create') {
      const request: CreateCatalogoPrecioRequest = {
        ...
        precios:     this.editForm.precios.map(p => ({ antiguedadAnios: p.antiguedadAnios, precioUsd: p.precioUsd })),
      };
      ...
      this.svc.update(this.editForm.id, request)
        .pipe(finalize(() => this.saveLoading.set(false)))
        .subscribe({
          next: () => {
  ...
  onEsc(): void {
    if (this.editPanelOpen()) { this.closeEdit(); return; }
    if (this.deletingId())    { this.deletingId.set(null); }
  }
}
