    ...
    <div style="font-family: var(--font-body);">
      ...
      <div class="card-elevated rounded-2xl p-6 mb-5">
        ...
        <input
          ...
          accept=".xlsx"
          (change)="onFile($event)"
          class="block w-full text-[13px] mb-4"
        />
        <div class="flex gap-2">
          <button
            (click)="validar()"
            [disabled]="!file || loading"
            class="px-4 py-2 rounded-xl bg-[#F3F4F6] border border-[#E4E7EC] text-[13px]"
          >
            Validar
          </button>
          <button
            (click)="importar()"
            [disabled]="!file || loading"
            class="px-4 py-2 rounded-xl bg-[#0D1017] text-white text-[13px]"
          >
            Importar
          </button>
        </div>
      ...
      @if (result) {
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          <div class="card-elevated rounded-xl p-4">
            <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Detectados</p>
            <p class="text-[24px] font-semibold">{{ result.registrosDetectados }}</p>
          </div>
          <div class="card-elevated rounded-xl p-4">
            <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Insertados</p>
            <p class="text-[24px] font-semibold">{{ result.insertados }}</p>
          </div>
          <div class="card-elevated rounded-xl p-4">
            <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Saltados</p>
            <p class="text-[24px] font-semibold">{{ result.saltados }}</p>
          </div>
          <div class="card-elevated rounded-xl p-4">
            <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Rechazados</p>
            <p class="text-[24px] font-semibold text-[#C61D26]">{{ result.rechazados }}</p>
          </div>
        </div>
        ...
        <div class="card-elevated rounded-2xl p-5">
          ...
          <pre
            class="text-[12px] leading-5 whitespace-pre-wrap bg-[#0D1017] text-white rounded-xl p-4 max-h-[420px] overflow-auto"
            >{{ logText() }}</pre
          >
        </div>
      }
...
export class ImportadorComponent {
  ...
  logText(): string {
    ...
    return [
      ...this.result.warnings.map(w => `WARNING: ${w}`),
      ...this.result.errores.map(e => `ERROR: ${e}`),
      ...this.result.log,
    ].join('\n');
  }
}
