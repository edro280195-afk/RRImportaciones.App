          ...
          <form (ngSubmit)="submit()" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="col-span-2 sm:col-span-1">
                <label
                  class="block text-[11px] font-semibold text-[#4B5162] uppercase tracking-[0.6px] mb-1.5"
                  >Fecha ingreso patio</label
                >
                <input
                  type="date"
                  [(ngModel)]="form.fechaIngresoPatio"
...
export class VehiculoFormDialogComponent {
  ...
  submit(): void {
    if (!this.form.vin?.trim()) {
      this.error.set('El VIN es requerido');
      return;
    }
    if (this.form.vin.trim().length !== 17) {
      this.error.set('El VIN debe tener exactamente 17 caracteres');
      return;
    }
