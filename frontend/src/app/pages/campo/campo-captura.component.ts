...
@Component({
    ...
    <main class="shell">
      ...
      @if (state() === 'ready' || state() === 'sending') {
        ...
        <div class="steps-bar">
          ...
          <div class="step" [class.step--done]="!!vinConfirmado" [class.step--active]="totalPhotoCount() >= MIN_PHOTOS && !vinConfirmado">
            <div class="step-circle">
              @if (vinConfirmado) {
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l3.5 3.5L13 5"/>
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <rect x="3" y="4" width="18" height="14" rx="2"/><path d="M7 9h3M7 13h2" stroke-linecap="round"/>
                </svg>
              }
            </div>
            <span>VIN<br><strong>{{ vinConfirmado || '——' }}</strong></span>
          </div>

          <div class="step-line" [class.line--done]="!!vinConfirmado"></div>

          <div class="step" [class.step--done]="!!ubicacion" [class.step--active]="!!vinConfirmado && !ubicacion">
            <div class="step-circle">
              @if (ubicacion) {
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l3.5 3.5L13 5"/>
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 21c-4-4-7-7.5-7-11a7 7 0 0114 0c0 3.5-3 7-7 11z"/><circle cx="12" cy="10" r="2.5"/>
                </svg>
              }
            </div>
            <span>Ubicación<br><strong>{{ ubicacion || '——' }}</strong></span>
          </div>
        </div>
        ...
        <button
          ...
          @if (totalPhotoCount() === 0) {
            ...
            <span class="hero-cam__sub">Faltan {{ MIN_PHOTOS - totalPhotoCount() }} fotos para continuar</span>
          } @else {
        ...
        <div class="fields-card">
          ...
          <label class="field-row">
            ...
            @if (vinConfirmado && t?.vinCorto) {
              <span class="vin-match" [class.vin-match--ok]="vinConfirmado.toUpperCase() === t!.vinCorto!.toUpperCase()" [class.vin-match--bad]="vinConfirmado.toUpperCase() !== t!.vinCorto!.toUpperCase()">
                {{ vinConfirmado.toUpperCase() === t!.vinCorto!.toUpperCase() ? '✓ Coincide con el sistema' : '⚠ No coincide — verifica el VIN' }}
              </span>
            }
          </label>
        </div>
        ...
        @if (!showIncidencia()) {
          ...
          <div class="incidencia-card">
            ...
            <textarea
              ...
              rows="3"
              placeholder="Describe el problema: daño visible, VIN no coincide, unidad no localizada..."></textarea>
          </div>
        }
  ...
  styles: [`
    ...
    @keyframes pulse-ring {
      0%   { box-shadow: 0 0 0 0 rgba(198,29,38,.35); }
      70%  { box-shadow: 0 0 0 14px rgba(198,29,38,0); }
      100% { box-shadow: 0 0 0 0 rgba(198,29,38,0); }
    }
    @keyframes bar-enter {
    ...
    .shell {
      ...
      padding: max(12px, env(safe-area-inset-top, 12px)) 14px max(160px, calc(env(safe-area-inset-bottom, 0px) + 140px));
      max-width: 600px;
      margin: 0 auto;
    }
    ...
    .chip--abierta  { background: var(--amber-lt); color: var(--amber); border: 1.5px solid #FCD34D; }
    .chip--tomada   { background: #EDE9FE; color: #7C3AED; border: 1.5px solid #C4B5FD; }
    .chip--en_yarda { background: #DBEAFE; color: #1D4ED8; border: 1.5px solid #93C5FD; }
    .chip--completada { background: var(--green-lt); color: var(--green); border: 1.5px solid #86EFAC; }
    ...
    .vin-ref__label { color: #1D4ED8; font-weight: 600; }
    .vin-ref__value { font-family: var(--font-mono); font-weight: 700; color: #1E40AF; letter-spacing: .04em; }
    ...
    .hero-cam {
      ...
      box-shadow: 0 6px 24px rgba(198,29,38,.3), 0 2px 6px rgba(198,29,38,.2);
      animation: slideUp .22s ease-out, pulse-ring 2.5s ease-out 1s 2;
      overflow: hidden;
    }
    .hero-cam::before {
      content: '';
    ...
    .hero-cam--has-photos  { background: var(--red-dk); animation: none; }
    .hero-cam--complete {
    ...
    .vin-match--ok  { background: var(--green-lt); color: var(--green); }
    .vin-match--bad { background: var(--amber-lt); color: var(--amber); }
    ...
    .btn-send {
      width: 100%;
      font-size: 17px;
      box-shadow: 0 4px 16px rgba(198,29,38,.3);
    }
    ...
    @media (max-width: 400px) {
      .hero-cam { padding: 24px 16px; }
      .hero-cam__icon { width: 60px; height: 60px; }
      .hero-cam__icon svg { width: 30px; height: 30px; }
      .hero-cam__title { font-size: 17px; }
      .vehicle-name { font-size: 15px; }
      .steps-bar { padding: 10px; gap: 0; }
      .step > span { display: none; }
      .step--active > span,
      .step--done   > span { display: block; }
    }
...
export class CampoCapturaComponent implements OnInit, OnDestroy {
  ...
  readonly totalPhotoCount = computed(() => this.photos().length + (this.tarea()?.fotosUrls?.length ?? 0));
  ...
  retryCamera(): void { void this.startCamera(); }
  ...
  async sendReport(): Promise<void> {
    ...
    try {
      ...
      await firstValueFrom(this.campoService.completar(current.id, {
      ...
      this.photos.update(items => items.map(p => p.uploading ? { ...p, uploading: false, err: true } : p));
      this.state.set('ready');
  ...
  persistDraft(): void {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({ ubicacion: this.ubicacion, vinConfirmado: this.vinConfirmado, incidencia: this.incidencia } satisfies CampoDraft),
    );
  }
  ...
  private loadDraft(id: string): void {
    ...
    try {
      ...
      if (this.incidencia) this.showIncidencia.set(true);
    } catch { localStorage.removeItem(this.draftKey(id)); }
  }
  ...
  private loadLocalPhotos(id: string): void {
    ...
    } catch { this.photos.set([]); localStorage.removeItem(this.photosKey(id)); }
  }
