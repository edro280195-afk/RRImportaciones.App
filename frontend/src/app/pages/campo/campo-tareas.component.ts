...
@Component({
    ...
    <div class="shell">
        ...
        <div class="brand">
          <div class="brand-logo">
            <svg viewBox="0 0 20 20" fill="none">
              ...
              <path d="M6 14L10 6L14 14" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <div>
      ...
      <div class="summary-banner" [class.banner--clear]="pendientes() === 0 && tareas().length > 0">
        @if (loading() && tareas().length === 0) {
          ...
          <div>
            <p class="summary-title">{{ pendientes() === 1 ? 'unidad pendiente' : 'unidades pendientes' }}</p>
      ...
      <section class="task-list" role="list">
        ...
        @for (t of filteredTareas(); track t.id) {
          <article
            ...
            <button class="task-tap" (click)="openCaptura(t)" [attr.aria-label]="'Capturar ' + (t.vehiculoResumen || t.numeroConsecutivo)">
              <div class="task-content">
                <div class="task-top">
                  <span class="task-folio">{{ t.numeroConsecutivo }}</span>
                  <span class="task-status-chip chip-{{ (t.estatus || 'abierta').toLowerCase() }}">{{ estadoLabel(t.estatus || '') }}</span>
                </div>
                  ...
                  @if (t.vinCorto) {
                    <span class="pill pill--vin">
                      <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.4">
                        <rect x="1" y="3" width="12" height="8" rx="1.5"/><path stroke-linecap="round" d="M3 6h3M3 8h2"/>
                      </svg>
                      {{ t.vinCorto }}
                    </span>
                  }
          ...
          @if (!loading()) {
            <div class="empty-state">
              ...
              <p class="empty-title">{{ activeFilter() === '' ? 'Sin tareas asignadas' : 'Sin tareas en este estado' }}</p>
              <p class="empty-sub">{{ activeFilter() !== '' ? 'Prueba seleccionando otro filtro' : 'Cuando te asignen una unidad aparecerá aquí' }}</p>
            </div>
          }
        }
      </section>
  ...
  styles: [`
    ...
    .summary-banner {
      ...
      padding: 18px 20px;
      color: #fff;
      box-shadow: 0 4px 16px rgba(198,29,38,.25);
    ...
    .card--abierta    { border-left: 4px solid var(--amber); }
    .card--tomada     { border-left: 4px solid var(--purple); }
    .card--yarda      { border-left: 4px solid var(--blue); }
    ...
    .chip-abierta    { background: var(--amber-lt);  color: var(--amber); }
    .chip-tomada     { background: var(--purple-lt); color: var(--purple); }
    .chip-en_yarda   { background: var(--blue-lt);   color: var(--blue); }
    .chip-incidencia { background: var(--red-lt);    color: var(--red); }
    .chip-completada { background: var(--green-lt);  color: var(--green); }
    .chip-cancelada  { background: #F3F4F6;          color: var(--text-3); }
    ...
    .pill--vin   { background: #F3F4F6;         color: var(--text-2); font-family: var(--font-mono); }
    ...
    .task-card:hover .arrow-cam { background: var(--red); border-color: var(--red); color: #fff; }
    ...
    @media (max-width: 380px) {
      .task-vehicle { font-size: 15px; }
      .summary-count { font-size: 42px; }
    }
  `],
})
export class CampoTareasComponent implements OnInit, OnDestroy {
  ...
  pendientes = computed(() =>
    this.tareas().filter(t => t.estatus !== 'COMPLETADA' && t.estatus !== 'CANCELADA' && t.estatus !== 'INCIDENCIA').length
  );
  ...
  load(): void {
    ...
    this.campoService.getTareas().subscribe({
      next:  tareas => { this.tareas.set(tareas); this.loading.set(false); },
      error: err    => { this.notifications.fromHttpError(err, 'Error al cargar tareas'); this.loading.set(false); },
    });
  }
  ...
  estadoLabel(value: string): string {
    const map: Record<string, string> = {
      ABIERTA: 'Abierta', TOMADA: 'Tomada', EN_YARDA: 'En yarda',
      INCIDENCIA: 'Incidencia', COMPLETADA: 'Completada', CANCELADA: 'Cancelada',
    };
