        ...
        >
          ...
          <form (ngSubmit)="submit()" class="space-y-4">
              ...
              <div class="col-span-2">
                ...
                >
                  ...
                  @for (v of vehiculos; track v.id) {
                    <option [value]="v.id">
                      {{ v.vinCorto || v.vin }} — {{ v.marcaNombre || '' }}
                      {{ v.modeloNombre || '' }}
                    </option>
                  }
                </select>
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-[#E4E7EC]">
              ...
              <button
                type="submit"
                [disabled]="saving() || !form.clienteId || !form.cobroTotal"
...
export class TramiteFormDialogComponent {
  ...
  private async loadCatalogs(): Promise<void> {
    ...
    if (clientesRes.ok) {
      ...
      this.clientes = d.items || d;
    }
    if (vehiculosRes.ok) {
      const d = await vehiculosRes.json();
      this.vehiculos = d.items || d;
    }
  ...
  submit(): void {
    ...
    if (!this.form.cobroTotal) {
      this.error.set('El cobro total es requerido');
      return;
    }
