    ...
    <div style="font-family: var(--font-body);">
      ...
      @if (tramite()) {
        ...
        @if (t.estatus === 'ENTREGADO_AL_CLIENTE') {
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded-full bg-[#DCFCE7] flex items-center justify-center shrink-0"
              >
                <svg
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  class="w-4 h-4 stroke-2 text-[#16A34A]"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
              ...
              <div>
                <p class="text-[13px] font-semibold text-[#166534]">
                  Trámite finalizado — Vehículo entregado
                </p>
                <p class="text-[12px] text-[#4B7C59]">
                  El vehículo fue entregado con su papelería. El historial operativo está cerrado.
                  Consulta Pagos para ver el saldo.
                </p>
              </div>
            </div>
            @if (t.entregas.length > 0) {
              <div class="sm:border-l border-[#BBF7D0] sm:pl-4">
                <p
                  class="text-[11px] font-semibold uppercase tracking-[0.6px] text-[#4B7C59] mb-0.5"
                >
                  Vehículo Entregado
                </p>
                <p class="text-[13px] font-semibold text-[#166534]">
                  {{ t.entregas[t.entregas.length - 1].fechaEntrega | date: 'dd/MM/yyyy HH:mm' }}
                </p>
                <p class="text-[11px] text-[#4B7C59]">
                  {{
                    t.entregas[t.entregas.length - 1].ubicacionEntrega ||
        ...
        @if (t.estatus === 'CANCELADO') {
          ...
          >
            <div
              class="w-8 h-8 rounded-full bg-[#FEE2E2] flex items-center justify-center shrink-0"
            >
              <svg
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                class="w-4 h-4 stroke-2 text-[#DC2626]"
              >
            ...
            <div>
              ...
              <p class="text-[12px] text-[#7F1D1D]">
                Este trámite fue cancelado. Consulta el timeline para ver el motivo registrado.
              </p>
            </div>
          </div>
        }
        ...
        @if (t.cotizacionOrigenId) {
          <div class="card-elevated mb-6 rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] p-4">
            <div class="flex items-center justify-between gap-3">
              <div>
                <p class="text-[13px] font-semibold text-[#1E40AF]">
                  Originado en cotizacion #{{ t.cotizacionOrigenFolio }}
                </p>
                <p class="text-[12px] text-[#4B5563]">
                  Cotizada {{ t.cotizacionFecha | date: 'dd/MM/yyyy' }} / convertida
                  {{ t.fechaCreacion | date: 'dd/MM/yyyy HH:mm' }}
                </p>
              </div>
              <button
                (click)="router.navigate(['/cotizaciones', t.cotizacionOrigenId])"
                class="rounded-xl bg-[#DBEAFE] px-3 py-2 text-[12px] font-semibold text-[#1E40AF]"
              >
                Ver cotizacion
              </button>
            </div>
        ...
        <div class="flex items-center gap-1 mb-4 overflow-x-auto pb-1 scrollbar-hide">
          @for (tab of tabs; track tab.key) {
            <button
              ...
              [style]="
                activeTab() === tab.key ? 'background: #0D1017; color: #fff;' : 'color: #6B717F;'
              "
            >
              {{ tab.label }}
            </button>
            ...
            @for (e of t.eventos; track e.id) {
              <div class="flex gap-3 pb-5 relative border-l-2 border-[#E4E7EC] pl-4 ml-2 last:pb-0">
                <div
                  ...
                  [style]="eventoColor(e.tipo)"
                ></div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-0.5">
                    <span class="text-[12px] font-semibold text-[#0D1017]">{{
                      tipoLabel(e.tipo)
                    }}</span>
                    <span class="text-[11px] text-[#9EA3AE] font-mono-data">{{
                      e.fechaEvento | date: 'dd/MM/yyyy HH:mm'
                    }}</span>
                  </div>
                  ...
                  <tbody>
                    @for (p of t.pedimentos; track p.id) {
                      <tr class="text-[13px] border-b border-[#F3F4F6]">
                        <td class="px-4 py-3 font-mono-data font-semibold">
                          {{ p.numeroPedimento }}
                        </td>
        ...
        @if (activeTab() === 'entregas') {
          <div class="card-elevated rounded-2xl p-6">
            <div class="flex justify-between items-center mb-6">
              ...
              <button
                (click)="showEntrega = true"
                class="px-4 py-2.5 rounded-xl text-[13px] font-semibold bg-[#111827] text-white hover:bg-[#1F2937] transition-all shadow-md shadow-gray-900/10"
              >
                Registrar Entrega
              </button>
            </div>
            ...
            @if (t.entregas.length === 0) {
              <div
                class="text-center py-12 rounded-xl border border-dashed border-[#D1D5DB] bg-[#F9FAFB]"
              >
                <div
                  class="w-12 h-12 bg-white rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm border border-[#E5E7EB]"
                >
                  <svg
                    fill="none"
                    stroke="currentColor"
            ...
            <div class="space-y-4">
              @for (e of t.entregas; track e.id) {
                <div
                  class="border border-[#E5E7EB] rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow"
                >
                  <div
                    class="px-5 py-4 bg-[#F9FAFB] border-b border-[#E5E7EB] flex flex-wrap justify-between items-start gap-4"
                  >
                    <div>
                      <span
                        class="inline-block px-2.5 py-1 bg-[#ECFDF5] text-[#059669] text-[11px] font-bold rounded-md uppercase tracking-[0.5px] mb-2 border border-[#A7F3D0]"
                        >Acuse Oficial</span
                      >
                      <h4 class="text-[15px] font-bold text-[#111827]">
                        {{ e.fechaEntrega | date: 'EEEE dd/MM/yyyy HH:mm' }}
                      </h4>
                      <p class="text-[13px] text-[#4B5563] mt-1">
                        Ubicación:
                        <span class="font-semibold text-[#111827]">{{
                          e.ubicacionEntrega || 'Yarda'
                        }}</span>
                      </p>
                    </div>
                    <div class="text-right">
                      <p
                        class="text-[12px] text-[#6B7280] uppercase tracking-[0.5px] font-semibold mb-1"
                      >
                        Entregado a
                      </p>
                      ...
                      <p class="text-[12px] text-[#6B7280] mt-1">
                        Registró:
                        {{
                          e.responsableCampoNombre || e.recibidoPorPartnerNombre || 'Administración'
                        }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div class="px-5 py-4 flex flex-col md:flex-row gap-6">
                    <div class="flex-1">
                      <h5
                        class="text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-2"
                      >
                      ...
                      @if (e.documentosEntregados && e.documentosEntregados.length) {
                        <div class="flex flex-wrap gap-2">
                          @for (doc of e.documentosEntregados; track doc) {
                            <span
                              class="px-3 py-1.5 bg-[#F3F4F6] text-[#374151] rounded-lg text-[13px] font-medium border border-[#E5E7EB] flex items-center gap-1.5"
                            >
                              <svg
                        ...
                        <p class="text-[13px] text-[#9CA3AF] italic">
                          No se registró lista de documentos.
                        </p>
                      }
                      ...
                      @if (e.descripcion) {
                        <div class="mt-4 pt-4 border-t border-[#F3F4F6]">
                          <h5
                            class="text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-1"
                          >
                            Notas
                          </h5>
                          <p class="text-[13px] text-[#374151] leading-relaxed">
                            {{ e.descripcion }}
                          </p>
                        </div>
                      }
                    ...
                    @if (e.fotoEvidenciaUrl || e.firmaBase64) {
                      <div class="w-full md:w-48 shrink-0 flex flex-col gap-2">
                        <h5
                          class="text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px]"
                        >
                          Evidencia
                        </h5>
                        @if (e.fotoEvidenciaUrl) {
                          <a
                            [href]="fileUrl(e.fotoEvidenciaUrl)"
                            target="_blank"
                            class="block w-full aspect-video rounded-xl border border-[#E5E7EB] overflow-hidden hover:border-[#111827] transition-colors relative group"
                          >
                            <img
                              [src]="fileUrl(e.fotoEvidenciaUrl)"
                ...
                <div class="rounded-xl border border-[#E4E7EC] p-4">
                  ...
                  <div class="flex items-start justify-between gap-3 mb-3">
                    ...
                    <span
                      ...
                      [style]="documentoPill(actual?.estadoLogistico || 'PENDIENTE')"
                      >{{ actual?.estadoLogistico || 'PENDIENTE' }}</span
                    >
                  </div>
                  <div class="flex flex-col gap-3 mt-3">
                    @if (actual?.archivoUrl) {
                      <div class="flex flex-col gap-2">
                        <div class="flex items-center gap-2">
                          <a
                            [href]="fileUrl(actual!.archivoUrl!)"
                            target="_blank"
                            class="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-[#F3F4F6] text-[#374151] rounded-xl text-[12px] font-semibold border border-[#E5E7EB] hover:bg-[#E5E7EB] transition-colors"
                          >
                            <svg
                              class="w-4 h-4"
                              fill="none"
                          ...
                          <label
                            class="px-3 py-2 bg-white text-[#4B5563] rounded-xl text-[12px] font-semibold border border-[#E5E7EB] cursor-pointer hover:bg-[#F9FAFB] transition-colors relative"
                            title="Reemplazar archivo"
                          >
                            @if (uploadingDoc === doc.tipoDocumento) {
                              <svg
                                class="w-4 h-4 animate-spin text-[#0D1017]"
                              ...
                              <input
                                ...
                                (change)="
                                  onDocumentoFileSelected($event, doc.tipoDocumento, 'RECIBIDO')
                                "
                              />
                            }
                          </label>
                      ...
                      <label
                        class="w-full flex flex-col items-center justify-center py-4 border-2 border-dashed border-[#E5E7EB] rounded-xl cursor-pointer bg-[#F9FAFB] hover:bg-[#F3F4F6] transition-colors relative"
                      >
                        @if (uploadingDoc === doc.tipoDocumento) {
                          <svg
                            class="w-6 h-6 animate-spin text-[#0D1017] mb-1"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                          >
                          ...
                          <svg
                            class="w-6 h-6 text-[#9CA3AF] mb-1"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                    ...
                    <button
                      (click)="
                        guardarDocumento(t.id, doc.tipoDocumento, 'VALIDADO', actual?.archivoUrl!)
                      "
                      [disabled]="!actual?.archivoUrl || actual?.estadoLogistico === 'VALIDADO'"
                      [style]="
                        actual?.estadoLogistico === 'VALIDADO'
                    ...
                    >
                      @if (actual?.estadoLogistico === 'VALIDADO') {
                        <svg
                          ...
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                          class="w-3.5 h-3.5 stroke-2 shrink-0"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                ...
                <div class="border-t border-[#F3F4F6] py-3 text-[13px]">
                  <div class="flex justify-between gap-3 mb-2">
                    <div>
                      ...
                      @if (tc.incidencia) {
                        <p
                          class="text-[#991B1B] mt-1 bg-[#FEF2F2] px-2 py-1 rounded-md inline-block border border-[#FEE2E2]"
                        >
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                      @for (foto of tc.fotosUrls; track foto) {
                        <a
                          [href]="fileUrl(foto)"
                          target="_blank"
                          class="block w-16 h-16 rounded-lg border border-[#E4E7EC] overflow-hidden hover:border-[#0D1017] transition-colors relative group"
                        >
          ...
          <div class="card-elevated rounded-2xl p-5">
            @if (auth.can('GASTOS_REGISTRAR')) {
              <div class="flex justify-end mb-3">
                <button
                  (click)="openGastoModal()"
                  class="px-3.5 py-2 rounded-xl text-[12px] font-medium bg-[#0D1017] text-white"
                >
                  Nuevo gasto
                </button>
              </div>
            ...
            @if (t.gastosHormiga.length === 0) {
              <p class="text-[13px] text-[#9EA3AE] text-center py-8">
                Sin gastos hormiga registrados
              </p>
            }
            @for (g of t.gastosHormiga; track g.id) {
              <div
                class="flex justify-between items-center border-b border-[#F3F4F6] py-2 last:border-0"
              >
                <div>
                  <p class="text-[13px]">{{ g.concepto }}</p>
                  <p class="text-[11px] text-[#9EA3AE]">
                    {{ g.fechaGasto | date: 'dd/MM/yyyy' }} | {{ g.tipoGasto || '—' }} |
                    {{ g.seCargaAlCliente ? 'Cargable' : 'Costo propio' }}
                  </p>
                </div>
                <p class="text-[13px] font-mono-data font-semibold">
        ...
        @if (activeTab() === 'pagos') {
          <div class="card-elevated rounded-2xl p-5">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div>
                <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Cobro total</p>
                <p class="text-[15px] font-semibold">{{ t.cobroTotal | currency: 'MXN' }}</p>
              </div>
              <div>
                <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Total pagado</p>
                <p class="text-[15px] font-semibold">{{ t.totalPagado | currency: 'MXN' }}</p>
              </div>
              <div>
                <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Verificado</p>
                <p class="text-[15px] font-semibold">{{ totalVerificado(t) | currency: 'MXN' }}</p>
              </div>
              <div>
                <p class="text-[11px] text-[#9EA3AE] uppercase font-semibold">Saldo</p>
                <p class="text-[15px] font-semibold">{{ t.saldoPendiente | currency: 'MXN' }}</p>
              </div>
            </div>
            ...
            <div class="overflow-x-auto">
              <div class="min-w-[700px]">
                @for (p of t.pagos; track p.id) {
                  <div
                    class="grid grid-cols-[100px_1fr_80px_105px_88px_64px_54px] gap-3 items-center border-b border-[#F3F4F6] py-2 last:border-0 text-[13px]"
                  >
                    ...
                    <span class="text-right font-mono-data">{{
                      p.monto | currency: p.moneda
                    }}</span>
                    <span
                      class="px-2 py-1 rounded-lg text-[11px] font-semibold text-center"
                      [style]="
                        p.verificado
                          ? 'background:#DCFCE7;color:#166534;'
                          : 'background:#FEF3C7;color:#92400E;'
                      "
                      >{{ p.verificado ? 'Verificado' : 'Pendiente' }}</span
                    >
                    <a
                      [href]="reciboUrl(p)"
                      target="_blank"
                      class="text-right text-[12px] font-medium text-[#C61D26]"
                      >Recibo</a
                    >
                    <button
                      (click)="deletePago(p.id, p.monto, p.moneda)"
                      class="text-right text-[12px] font-medium text-[#991B1B]"
                    >
                      Borrar
                    </button>
                  </div>
      ...
      @if (showCambiarEstado) {
        <div
          class="fixed inset-0 bg-black/30 z-50 flex items-start justify-center pt-[15vh] px-4 overflow-y-auto"
          (click)="showCambiarEstado = false"
        >
          <div
            class="bg-white rounded-2xl p-6 w-full max-w-[400px] shadow-xl my-auto"
            (click)="$event.stopPropagation()"
          >
            <h3 class="text-[16px] font-semibold mb-4">Cambiar estado</h3>
            <select
              [(ngModel)]="nuevoEstado"
              class="w-full px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] mb-3"
            >
            ...
            <textarea
              ...
              class="w-full px-3 py-2 rounded-xl border border-[#E4E7EC] text-[13px] mb-3"
              rows="2"
            ></textarea>
            <div class="flex justify-end gap-2">
              <button
                (click)="showCambiarEstado = false"
                class="px-4 py-2 rounded-xl text-[12.5px] border border-[#E4E7EC]"
              >
                Cancelar
              </button>
              <button
                (click)="doCambiarEstado()"
                class="px-4 py-2 rounded-xl text-[12.5px] bg-[#0D1017] text-white"
              >
                Confirmar
              </button>
            </div>
      ...
      @if (showPedimento) {
        <div
          class="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50 flex items-start justify-center pt-[8vh] px-4 overflow-y-auto"
          (click)="showPedimento = false"
        >
          <div
            class="bg-white rounded-[24px] p-6 md:p-8 w-full max-w-[480px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#E4E7EC] my-auto"
            (click)="$event.stopPropagation()"
          >
            <div class="mb-6">
              <h3 class="text-[20px] font-bold text-[#0D1017] tracking-[-0.5px]">
                Agregar Pedimento
              </h3>
            ...
            <div class="space-y-4">
              <div>
                <label
                  class="block text-[12px] font-semibold text-[#9EA3AE] uppercase tracking-[0.5px] mb-1.5 ml-1"
                  >Número de pedimento</label
                >
                <input
                  [(ngModel)]="pedForm.numero"
                  placeholder="Ej. 24 00 3000 4000123"
                  class="w-full px-4 py-3 rounded-xl border border-[#E4E7EC] text-[14px] font-mono-data focus:border-[#C61D26] focus:ring-4 focus:ring-[#C61D26]/5 outline-none transition-all bg-[#F9FAFB]"
                />
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                  <select
                    [(ngModel)]="pedForm.tipo"
                    class="w-full px-4 py-3 rounded-xl border border-[#E4E7EC] text-[14px] focus:border-[#C61D26] focus:ring-4 focus:ring-[#C61D26]/5 outline-none transition-all bg-white"
                  >
                ...
                <div>
                  ...
                  >
                  <input
                    [(ngModel)]="pedForm.cobroAdicional"
                    type="number"
                    step="0.01"
                    class="w-full px-4 py-3 rounded-xl border border-[#E4E7EC] text-[14px] focus:border-[#C61D26] outline-none bg-white"
                  />
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                  <input
                    [(ngModel)]="pedForm.fechaEntrada"
                    type="date"
                    class="w-full px-4 py-3 rounded-xl border border-[#E4E7EC] text-[14px] outline-none bg-white"
                  />
                </div>
                <div>
                  <label
            ...
            <div class="flex items-center gap-3 mt-8">
              <button
                (click)="showPedimento = false"
                class="flex-1 px-4 py-3 rounded-xl text-[14px] font-semibold text-[#4B5162] bg-[#F3F4F6] hover:bg-[#E4E7EC] transition-colors"
              >
                Cancelar
              </button>
              <button
                (click)="doAgregarPedimento()"
                [disabled]="!pedForm.numero"
                class="flex-1 px-4 py-3 rounded-xl text-[14px] font-semibold bg-[#0D1017] text-white hover:bg-[#1E2330] transition-colors disabled:opacity-40"
              >
                Guardar
              </button>
            </div>
      ...
      @if (showEntrega) {
        <div
          class="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-[8vh] px-4 overflow-y-auto backdrop-blur-sm"
          (click)="showEntrega = false"
        >
          <div
            class="bg-white rounded-2xl w-full max-w-[600px] shadow-2xl my-auto overflow-hidden flex flex-col"
            (click)="$event.stopPropagation()"
          >
            <div
              class="px-6 py-5 border-b border-[#F3F4F6] flex justify-between items-center bg-[#FAFAFA]"
            >
              <div>
                <h3 class="text-[18px] font-bold text-[#111827]">
                  Acuse de Recibo / Entrega Oficial
                </h3>
                <p class="text-[13px] text-[#6B7280] mt-1">
                  Registra la entrega formal del vehículo y documentos
                </p>
              </div>
              <button
                (click)="showEntrega = false"
                class="text-[#9CA3AF] hover:text-[#4B5563] transition-colors p-2"
              >
            ...
            <div class="px-6 py-5 overflow-y-auto max-h-[60vh]">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                  <label
                    class="block text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-1.5 ml-1"
                    >Ubicación de Entrega</label
                  >
                  <input
                    [(ngModel)]="entForm.ubicacion"
                    placeholder="Ej. Yarda Nuevo Laredo"
                    class="w-full px-4 py-3 rounded-xl border border-[#E5E7EB] text-[14px] outline-none focus:border-[#C61D26] focus:ring-4 focus:ring-[#C61D26]/5 transition-all bg-white"
                  />
                </div>
                <div>
                  <label
                    class="block text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-1.5 ml-1"
                    >Entregado a (Nombre Completo)</label
                  >
                  <input
                    [(ngModel)]="entForm.nombreRecibe"
              ...
              <div class="mb-5">
                <label
                  class="block text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-2 ml-1"
                  >Documentación Entregada</label
                >
                <div
                  class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 border border-[#E5E7EB] rounded-xl bg-[#F9FAFB]"
                >
                  <label class="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      [(ngModel)]="entForm.documentos.vehiculo"
                      class="w-5 h-5 rounded border-[#D1D5DB] text-[#C61D26] focus:ring-[#C61D26]"
                    />
                  ...
                  <label class="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      [(ngModel)]="entForm.documentos.pedimento"
                      class="w-5 h-5 rounded border-[#D1D5DB] text-[#C61D26] focus:ring-[#C61D26]"
                    />
                  ...
                  <label class="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      [(ngModel)]="entForm.documentos.factura"
                      class="w-5 h-5 rounded border-[#D1D5DB] text-[#C61D26] focus:ring-[#C61D26]"
                    />
                  ...
                  <label class="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      [(ngModel)]="entForm.documentos.titulo"
                      class="w-5 h-5 rounded border-[#D1D5DB] text-[#C61D26] focus:ring-[#C61D26]"
                    />
              ...
              <div class="mb-5">
                <label
                  class="block text-[12px] font-semibold text-[#4B5563] uppercase tracking-[0.5px] mb-1.5 ml-1"
                  >Evidencia (Foto o Firma escaneada)</label
                >
                <input
                  type="file"
                  (change)="onEntregaFileSelected($event)"
                  accept="image/*"
                  class="w-full px-4 py-3 rounded-xl border border-[#E5E7EB] text-[14px] bg-white cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[13px] file:font-semibold file:bg-[#FEE2E2] file:text-[#B91C1C] hover:file:bg-[#FECACA]"
                />
                @if (entForm.fotoUrl) {
                  <p class="text-[12px] text-green-600 font-semibold mt-2 ml-1">
              ...
              <div>
                ...
                >
                <input
                  [(ngModel)]="entForm.descripcion"
                  placeholder="Ej. El cliente se retiró conforme..."
                  class="w-full px-4 py-3 rounded-xl border border-[#E5E7EB] text-[14px] outline-none focus:border-[#C61D26] transition-all bg-white"
                />
              </div>
            </div>

            <div
              class="px-6 py-5 border-t border-[#F3F4F6] bg-[#FAFAFA] flex items-center justify-end gap-3"
            >
              <button
                (click)="showEntrega = false"
                class="px-5 py-2.5 rounded-xl text-[14px] font-semibold text-[#4B5563] hover:bg-[#E5E7EB] transition-colors"
              >
                Cancelar
              </button>
              <button
                (click)="doAgregarEntrega()"
                class="px-6 py-2.5 rounded-xl text-[14px] font-semibold bg-[#111827] text-white hover:bg-[#1F2937] transition-colors shadow-lg shadow-gray-900/20 disabled:opacity-50"
                [disabled]="!entForm.nombreRecibe || !entForm.ubicacion"
              >
                Registrar Entrega
              </button>
            </div>
      ...
      @if (showNota) {
        <div
          class="fixed inset-0 bg-black/30 z-50 flex items-start justify-center pt-[15vh] px-4 overflow-y-auto"
          (click)="showNota = false"
        >
          <div
            class="bg-white rounded-2xl p-6 w-full max-w-[450px] shadow-xl my-auto"
            (click)="$event.stopPropagation()"
          >
            <h3 class="text-[16px] font-semibold mb-4">Agregar nota</h3>
            <textarea
              [(ngModel)]="notaContenido"
      ...
      @if (showPago) {
        <div
          ...
          (click)="!savingPago && (showPago = false)"
        >
          <form
            class="bg-white rounded-2xl p-6 w-full max-w-[560px] shadow-xl my-auto"
            (click)="$event.stopPropagation()"
            (ngSubmit)="doRegistrarPago()"
          >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              />
              <select
                [(ngModel)]="pagoForm.moneda"
                name="moneda"
                (change)="onPagoMonedaChange()"
                class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px]"
              >
              ...
              <input
                [(ngModel)]="pagoForm.tipoCambio"
                name="tipoCambio"
                type="number"
                step="0.000001"
                [readonly]="pagoForm.moneda !== 'USD'"
                [placeholder]="pagoForm.moneda === 'USD' ? 'TC automatico' : 'Solo USD'"
                class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] read-only:bg-[#F3F4F6]"
              />
              <select
                [(ngModel)]="pagoForm.metodo"
                name="metodo"
                class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px]"
              >
                ...
                >
                  ...
                  @for (b of bancos; track b.id) {
                    <option [value]="b.nombre">
                      {{ b.identificador }} · {{ b.nombre }}{{ b.cuenta ? ' · ' + b.cuenta : '' }}
                    </option>
                  }
                </select>
              } @else {
            ...
            <input
              [(ngModel)]="pagoForm.referencia"
              name="referencia"
              placeholder="Referencia"
              class="w-full px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] mt-2"
            />
            <div
            ...
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <strong class="font-mono-data text-[#0D1017]">{{
                  saldoDisponiblePago() | currency: 'MXN'
                }}</strong>
              </div>
              <div class="flex items-center justify-between gap-3 mt-1">
                <span>Este pago en MXN</span>
                <strong class="font-mono-data" [class.text-[#991B1B]]="pagoExcedeSaldo()">{{
                  pagoFormMxn() | currency: 'MXN'
                }}</strong>
              </div>
            </div>
            ...
            <textarea
              [(ngModel)]="pagoForm.notas"
              name="notas"
              placeholder="Notas"
              class="w-full px-3 py-2 rounded-xl border border-[#E4E7EC] text-[13px] mt-2"
              rows="2"
            ></textarea>
            <label class="mt-3 block text-[12px] text-[#6B717F]">{{
            ...
            <input
              ...
              (change)="onPagoFile($event)"
              class="mt-1 text-[13px]"
            />
            @if (pagoFileName) {
              <p class="text-[12px] text-[#6B717F] mt-1">Comprobante: {{ pagoFileName }}</p>
            }
            <div class="flex justify-end gap-2 mt-5">
              <button
                type="button"
                [disabled]="savingPago"
                (click)="showPago = false"
              ...
              <button
                type="submit"
                [disabled]="savingPago || pagoValidationMessages().length > 0"
      ...
      @if (showFinalizarModal && tramite()) {
        @let t = tramite()!;
        <div
          class="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50 flex items-center justify-center px-4"
          (click)="showFinalizarModal = false"
        >
          <div
            class="bg-white rounded-[24px] w-full max-w-[420px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-[#E4E7EC]"
            (click)="$event.stopPropagation()"
          >
              ...
              <div class="flex items-center gap-3 mb-1">
                <div class="w-9 h-9 rounded-full bg-[#DCFCE7] flex items-center justify-center">
                  <svg
                    ...
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    class="w-5 h-5 stroke-2 text-[#16A34A]"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                ...
                <div>
                  ...
                  <p class="text-[12px] text-[#6B717F]">
                    {{ t.numeroConsecutivo }} · {{ t.clienteApodo || '—' }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="px-6 py-4 space-y-3">
              <div
                class="rounded-xl border border-[#E4E7EC] bg-[#F8FAFC] px-4 py-3 grid grid-cols-2 gap-y-2 text-[13px]"
              >
                <span class="text-[#6B717F]">Cobro total</span>
                <span class="text-right font-semibold font-mono-data">{{
                  t.cobroTotal | currency: 'MXN'
                }}</span>
                <span class="text-[#6B717F]">Total pagado</span>
                <span class="text-right font-semibold font-mono-data">{{
                  t.totalPagado | currency: 'MXN'
                }}</span>
                <span class="text-[#6B717F]">Saldo pendiente</span>
                <span
                  class="text-right font-semibold font-mono-data"
                  [style.color]="t.saldoPendiente > 0 ? '#D97706' : '#16A34A'"
                  >{{ t.saldoPendiente | currency: 'MXN' }}</span
                >
              </div>

              <div
                class="rounded-xl border border-[#DBEAFE] bg-[#EFF6FF] px-4 py-3 flex items-start gap-2"
              >
                <svg
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  class="w-4 h-4 stroke-2 text-[#2563EB] mt-0.5 shrink-0"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                ...
                <p class="text-[12.5px] text-[#1E40AF]">
                  Al confirmar, el trámite quedará en estado
                  <strong>ENTREGADO_AL_CLIENTE</strong> (finalizado). Debes tener al menos una
                  entrega registrada.
                </p>
              </div>
              ...
              <textarea
                ...
                rows="2"
                class="w-full px-3 py-2 rounded-xl border border-[#E4E7EC] text-[13px] resize-none focus:outline-none focus:border-[#16A34A]"
              ></textarea>
            </div>
            ...
            <div class="px-6 pb-6 flex gap-2">
              <button
                (click)="showFinalizarModal = false"
                class="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-semibold border border-[#D8DEE8] hover:bg-[#F3F4F6] transition-colors"
              >
                Cancelar
              </button>
              <button
                (click)="doFinalizar()"
                class="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-semibold bg-[#16A34A] text-white hover:bg-[#15803D] transition-colors"
              >
                Confirmar entrega
              </button>
            </div>
      ...
      @if (showGasto) {
        <div
          class="fixed inset-0 bg-black/30 z-50 flex items-start justify-center pt-[8vh] px-4 overflow-y-auto"
          (click)="showGasto = false"
        >
          <form
            class="bg-white rounded-2xl p-6 w-full max-w-[520px] shadow-xl my-auto"
            (click)="$event.stopPropagation()"
            (ngSubmit)="doRegistrarGasto()"
          >
            <h3 class="text-[16px] font-semibold mb-4">Nuevo gasto</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <input
                [(ngModel)]="gastoForm.fechaGasto"
                name="fechaGasto"
                type="date"
                class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px]"
              />
              <select
                [(ngModel)]="gastoForm.tipoGastoId"
                name="tipoGastoId"
                class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px]"
              >
            ...
            <input
              [(ngModel)]="gastoForm.concepto"
              name="concepto"
              placeholder="Concepto"
              class="w-full px-3 py-2.5 rounded-xl border border-[#E4E7EC] text-[13px] mt-2"
            />
            <label class="flex items-center gap-2 text-[13px] mt-3"
              ><input
                [(ngModel)]="gastoForm.seCargaAlCliente"
                name="seCargaAlCliente"
                type="checkbox"
              />
              Se carga al cliente</label
            >
            <div class="flex justify-end gap-2 mt-5">
              <button
                type="button"
...
export class TramiteDetailComponent implements OnInit {
  ...
  pagoFormMxn(): number {
    ...
    if (this.pagoForm.moneda === 'USD')
      return this.pagoForm.monto * (this.pagoForm.tipoCambio || 0);
  ...
  pagoValidationMessages(): string[] {
    ...
    if (
      this.pagoForm.moneda === 'USD' &&
      (!this.pagoForm.tipoCambio || this.pagoForm.tipoCambio <= 0)
    )
      messages.push('El tipo de cambio es obligatorio para pagos en USD.');
    if (
      (this.pagoForm.metodo === 'TRANSFERENCIA' || this.pagoForm.metodo === 'DEPOSITO') &&
      !this.pagoForm.banco
    )
      messages.push('Selecciona el banco para transferencias o depositos.');
    if (this.pagoForm.metodo !== 'EFECTIVO' && !this.pagoFile)
      messages.push('Adjunta el comprobante bancario.');
    if (this.pagoExcedeSaldo())
      messages.push(
        `El pago excede el saldo disponible por ${(this.pagoFormMxn() - this.saldoDisponiblePago()).toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}.`
      );
    return messages;
  }
  ...
  async deletePago(id: string, monto: number, moneda: string): Promise<void> {
    ...
    const confirmed = await this.notifications.confirm({
      title: 'Borrar pago',
      message:
        'Se ocultara del tramite y de los saldos operativos. La bitacora administrativa conservara el movimiento.',
  ...
  doRegistrarGasto() {
    const t = this.tramite();
    if (!t || !this.gastoForm.tipoGastoId || !this.gastoForm.concepto || this.gastoForm.monto <= 0)
      return;
      ...
      .subscribe({
        next: () => {
          ...
          this.entForm = {
            ...
            ubicacion: '',
            nombreRecibe: '',
            documentos: { vehiculo: true, pedimento: false, factura: false, titulo: false },
            fotoUrl: '',
            firmaBase64: '',
          };
