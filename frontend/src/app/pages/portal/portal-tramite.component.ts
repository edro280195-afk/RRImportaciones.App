...
@Component({
      ...
      @if (loading()) {
          ...
          <div class="vehicle-card-photo-wrapper">
            @if (vehiclePhoto()) {
              <img
                ...
                class="vehicle-photo"
                (click)="openLightbox(vehiclePhoto()!, t.vehiculoResumen)"
              />
            } @else {
        ...
        <nav class="sticky-tabs">
          <button
            ...
            (click)="activeTab.set('SEGUIMIENTO')"
          >
                ...
                @for (item of visibleTimeline(); track item.id; let last = $last) {
                  <div class="timeline-node" [class.pending]="!item.completado" [class.last]="last">
                    ...
                    <div class="node-content-card">
                      ...
                      @if (item.fotoUrl) {
                        <div
                          class="node-photo-preview"
                          (click)="openLightbox(fileUrl(item.fotoUrl), item.titulo)"
                        >
                ...
                >
                  {{
                    timelineExpanded()
                      ...
                      : 'Ver historial completo (' + t.timeline.length + ' pasos)'
                  }}
                </button>
              }
            </article>
            ...
            <article class="tab-panel payment-panel">
              ...
              <div class="financial-card-hero">
                <div class="financial-info">
                  ...
                  <h3 class="amount-pending">
                    {{ t.pagosResumen.pendiente | currency: 'MXN' : 'symbol' : '1.2-2' }}
                  </h3>
                </div>
              ...
              <div class="financial-stats-grid">
                ...
                <div class="stat-card paid">
                  ...
                  <strong>{{
                    t.pagosResumen.pagado | currency: 'MXN' : 'symbol' : '1.2-2'
                  }}</strong>
                </div>
              </div>
              ...
              @if (t.pagosResumen.pendienteVerificacion > 0) {
                <div class="alert-notice warning">
                  ...
                  <span
                    >Hay
                    <strong>{{
                      t.pagosResumen.pendienteVerificacion | currency: 'MXN' : 'symbol' : '1.2-2'
                    }}</strong>
              ...
              @if (t.pagosResumen.cubiertoPorRr > 0) {
                <div class="alert-notice info">
                  ...
                  <span
                    >R&R cubrió un saldo de
                    <strong>{{
                      t.pagosResumen.cubiertoPorRr | currency: 'MXN' : 'symbol' : '1.2-2'
                    }}</strong>
                    para agilizar tu cruce.</span
                  >
                </div>
              }
                        ...
                        <div class="payment-details">
                          <div class="payment-primary">
                            <span class="payment-method"
                              >{{ p.metodo }}{{ p.banco ? ' • ' + p.banco : '' }}</span
                            >
                            <span class="payment-date">{{
                              p.fechaPago | date: 'dd MMM yyyy'
                            }}</span>
                          </div>
                          @if (p.referencia) {
              ...
              @if (t.entrega) {
                <div class="official-delivery-card">
                  <div class="delivery-badge-ribbon">
                    <svg fill="currentColor" viewBox="0 0 24 24">
                      <path
                        d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm-1.177-7.86l-2.765-2.767L7 12.431l3.823 3.824 7.653-7.568-1.06-1.06-6.593 6.513z"
                      />
                    </svg>
                    ...
                    @if (t.entrega.fotoEvidenciaUrl) {
                      <div class="delivery-evidence-container">
                        ...
                        <div
                          ...
                          (click)="
                            openLightbox(
                              fileUrl(t.entrega.fotoEvidenciaUrl),
                              'Evidencia de Entrega'
                            )
                          "
                        >
                          <img
                            [src]="fileUrl(t.entrega.fotoEvidenciaUrl)"
  ...
  styles: [
    `
      ...
      .delivery-meta-grid .label,
      .delivery-docs-box .label,
      .delivery-evidence-container .label {
        font-size: 10px;
        font-weight: 700;
        text-transform: uppercase;
        color: #047857;
...
export class PortalTramiteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.portalService.getTramite(token).subscribe({
      ...
      error: err => {
        this.error.set(
          err?.error?.message ?? 'Revisa el enlace o contacta a R&R para confirmar tu seguimiento.'
        );
        this.loading.set(false);
      },
    });
