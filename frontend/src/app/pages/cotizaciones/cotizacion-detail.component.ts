...
import { PlantillaMensajeDto, PlantillaMensajeService } from '../../services/plantilla-mensaje.service';
  ...
  template: `
    @if (cotizacion(); as c) {
      <div>
        ...
        <div class="mb-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            @if (c.estado !== 'CONVERTIDA') {
              <button (click)="recalcular()" [disabled]="sending()"
                ...
                @if (sending()) {
                  <svg class="animate-spin w-3.5 h-3.5 text-[#6B717F]" fill="none" viewBox="0 0 24 24">
        ...
        @if (c.estado === 'ACEPTADA' && !c.tramiteId) {
          <div class="mb-5 rounded-2xl border border-[#BBF7D0] bg-[#F0FDF4] px-5 py-4">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-xl bg-[#16A34A] flex items-center justify-center shrink-0">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                </svg>
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-[13.5px] font-semibold text-[#14532D] leading-tight">Cotización aceptada</p>
                <p class="text-[12.5px] text-[#166534] mt-0.5">Lista para convertirse en trámite formal.</p>
              </div>
              <button (click)="openConvertir()"
              ...
              <div>
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  @if (c.tipoCambioContexto === 'DOF') {
                    <span class="rounded bg-[#DCFCE7] px-1 py-0.5 text-[10px] font-bold text-[#166534]" title="Oficial Diario Oficial de la Federacion">DOF</span>
                  } @else if (c.tipoCambioContexto === 'FIX') {
                    <span class="rounded bg-[#FEF9C3] px-1 py-0.5 text-[10px] font-bold text-[#854D0E]" title="Referencia Banxico FIX">FIX</span>
                  }
                </div>
                @if (c.tipoCambioNota) {
                   <p class="text-[10px] text-[#6B717F] leading-tight mt-1">{{ c.tipoCambioNota }}</p>
                }
              </div>
        ...
        @if (emailOpen()) {
          <div class="fixed inset-0 z-40 flex items-start justify-center bg-black/35 px-4 pt-[10vh] overflow-y-auto">
            <div class="w-full max-w-[620px] rounded-2xl bg-white p-5 shadow-2xl my-auto">
              ...
              @if (plantillasEmail().length > 0) {
                <label class="mb-1 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#6B717F]">Plantilla</label>
                <select (change)="aplicarPlantillaEmail($event)" class="mb-3 w-full rounded-xl border border-[#E4E7EC] bg-white px-3 py-2.5 text-[13px] outline-none focus:border-[#C61D26]">
              ...
              <label class="mb-1 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#6B717F]">Mensaje</label>
              <textarea [(ngModel)]="emailMensaje" rows="5" class="w-full rounded-xl border border-[#E4E7EC] px-3 py-2.5 text-[13px] outline-none focus:border-[#C61D26]"></textarea>
        ...
        @if (whatsappOpen()) {
          <div class="fixed inset-0 z-40 flex items-start justify-center bg-black/35 px-4 pt-[5vh] overflow-y-auto">
            <div class="w-full max-w-[660px] rounded-2xl bg-white p-5 shadow-2xl my-auto">
              ...
              @if (plantillasWhatsApp().length > 0) {
                <label class="mb-1 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#6B717F]">Plantilla</label>
                <select (change)="aplicarPlantillaWhatsApp($event)" class="mb-3 w-full rounded-xl border border-[#E4E7EC] bg-white px-3 py-2.5 text-[13px] outline-none focus:border-[#16A34A]">
                  <option value="">— Sin plantilla —</option>
        ...
        @if (convertOpen()) {
          <div class="fixed inset-0 z-40 flex items-start justify-center bg-black/35 px-4 pt-[8vh] overflow-y-auto"
               (click)="convertOpen.set(false)">
            <div class="w-full max-w-[600px] rounded-2xl bg-white shadow-2xl my-auto"
              ...
              <div class="px-6 pt-6 pb-4 border-b border-[#E4E7EC]">
                <div class="flex items-start justify-between gap-4">
                  <div>
                    <h2 class="text-[18px] font-semibold text-[#0D1017] tracking-[-0.3px]">Iniciar trámite</h2>
                    <p class="text-[13px] text-[#6B717F] mt-0.5">La cotización se vinculará a este trámite.</p>
                  </div>
                  <button (click)="convertOpen.set(false)"
              ...
              <div class="px-6 py-4 bg-[#F8FAFC] border-b border-[#E4E7EC]">
                <p class="text-[10.5px] font-semibold uppercase tracking-[0.8px] text-[#9EA3AE] mb-2.5">Datos heredados de la cotización</p>
                <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-[13px]">
                  <div class="flex justify-between gap-2">
                    <span class="text-[#6B717F]">Vehículo</span>
                    <span class="font-medium text-[#0D1017] text-right">{{ c.marca }} {{ c.modelo }} {{ c.anno || '' }}</span>
                  </div>
                  <div class="flex justify-between gap-2">
                    <span class="text-[#6B717F]">VIN</span>
                    <span class="font-mono-data font-medium text-[#0D1017]">{{ c.vin || 'Sin VIN' }}</span>
                  </div>
                  <div class="flex justify-between gap-2">
                    <span class="text-[#6B717F]">Cobro total</span>
                    <span class="font-mono-data font-semibold text-[#0D1017]">\${{ c.total | number:'1.2-2' }}</span>
                  </div>
                  <div class="flex justify-between gap-2">
                    <span class="text-[#6B717F]">Honorarios</span>
                    <span class="font-mono-data font-medium text-[#0D1017]">\${{ c.honorarios | number:'1.2-2' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="px-6 py-5">
                <p class="text-[10.5px] font-semibold uppercase tracking-[0.8px] text-[#9EA3AE] mb-3.5">Datos operacionales</p>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#4B5162]">
                      Aduana <span class="text-[#DC2626]">*</span>
                    </label>
                    <select [(ngModel)]="convertForm.aduanaCodigo"
                  ...
                  <div>
                    ...
                    <select [(ngModel)]="convertForm.tramitadorId"
                      class="w-full rounded-xl border border-[#E4E7EC] bg-[#F9FAFB] px-3 py-2.5 text-[13px] outline-none focus:bg-white focus:border-[#C61D26] focus:shadow-[0_0_0_3px_rgba(198,29,38,0.10)] transition-all">
                  ...
                  <div>
                    <label class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#4B5162]">Tipo de trámite</label>
                    <select [(ngModel)]="convertForm.tipoTramite"
                      class="w-full rounded-xl border border-[#E4E7EC] bg-[#F9FAFB] px-3 py-2.5 text-[13px] outline-none focus:bg-white focus:border-[#C61D26] focus:shadow-[0_0_0_3px_rgba(198,29,38,0.10)] transition-all">
                  ...
                  <div>
                    <label class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.6px] text-[#4B5162]">Notas adicionales</label>
                    <textarea [(ngModel)]="convertForm.notasAdicionales" rows="2"
                      class="w-full rounded-xl border border-[#E4E7EC] bg-[#F9FAFB] px-3 py-2.5 text-[13px] outline-none focus:bg-white focus:border-[#C61D26] focus:shadow-[0_0_0_3px_rgba(198,29,38,0.10)] transition-all resize-none"></textarea>
                  </div>
                </div>
              </div>

              <!-- Footer -->
              <div class="px-6 pb-6 flex items-center justify-end gap-3 border-t border-[#E4E7EC] pt-4">
                <button (click)="convertOpen.set(false)"
                ...
                <button (click)="convertirATramite()"
                  [disabled]="converting() || !convertForm.aduanaCodigo || !convertForm.tramitadorId"
