...
import { CandidatoPrecio, CandidatosPrecioOutput, CotizacionInput, CotizacionOutput, CotizacionService } from '../../services/cotizacion.service';
...
@Component({
          ...
          <section class="card vin-card" [class.vin-card--decoded]="decodeOk()">
            ...
            @if (decoding()) {
              ...
              <div class="vin-status vin-status--ok">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7l3 3 7-7" stroke="#16A34A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                Decodificado: <strong>{{ decodeMessage() }}</strong>
              </div>
            } @else if (decodeMessage()) {
              <div class="vin-status vin-status--err">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="6" stroke="#991B1B" stroke-width="1.5"/><path d="M7 4v3M7 10h.01" stroke="#991B1B" stroke-width="1.5" stroke-linecap="round"/></svg>
                {{ decodeMessage() }}
              </div>
            }
          </section>
          ...
          <section class="card">
            ...
            @if (decodeOk() && !detailsOpen()) {
              <div class="vehicle-summary">
                <div><span>Marca</span><strong>{{ form.marca || '—' }}</strong></div>
                <div><span>Modelo</span><strong>{{ form.modelo || '—' }}</strong></div>
                <div><span>Año</span><strong>{{ form.anno || '—' }}</strong></div>
                <div><span>Cilindrada</span><strong>{{ form.cilindradaCm3 ? form.cilindradaCm3 + ' cc' : '—' }}</strong></div>
              </div>
            } @else {
              <div class="grid-2">
                <div class="field">
          ...
          <section class="card">
            ...
            <div class="grid-3">
              <div class="field">
                ...
                <input [ngModel]="tcMargen()" (ngModelChange)="onTcMargenChange($event)" type="number" step="0.01" />
            ...
            <button type="button" class="ghost-btn ghost-btn--inline" (click)="advancedOpen = !advancedOpen">
            ...
            @if (advancedOpen) {
              <div class="grid-2 advanced">
                <div class="field">
                  ...
                  <input [(ngModel)]="form.valorAduanaUsdOverride" type="number" placeholder="Sobrescribe Anexo 2" />
                </div>
                <div class="field">
                  <label>Override honorarios (MXN)</label>
                  <input [(ngModel)]="form.honorariosOverride" type="number" placeholder="Default según régimen" />
                </div>
                <div class="field">
            ...
            <div class="calc-row">
              <button (click)="buscarOCalcular()" [disabled]="cargando() || !canCalculate()" class="btn-calc">
                @if (cargando()) {
                  <span class="spinner spinner--white"></span>
                  {{ buscandoCandidatos() ? 'Buscando en catálogo…' : 'Calculando…' }}
                } @else {
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7l3 3 5-6" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                  Calcular cotización
                }
              </button>
              @if (calcError()) {
          ...
          <section class="card">
            ...
            <div class="grid-2">
              <div class="field relative">
                ...
                @if (clienteId) {
                  <button type="button" (click)="clearCliente()" class="ghost-btn ghost-btn--inline">Quitar</button>
                }
              </div>
              <div class="field">
        ...
        <aside class="cot-result">
          @if (cargando()) {
            <div class="result-loading">
              ...
              <p class="loader-help">Se consulta NHTSA, Anexo 2, Banxico y parámetros fiscales del régimen.</p>
            </div>
            ...
            <div class="cand-header">
              ...
              <p class="cand-sub">
                ...
                ({{ candidatos()!.antiguedadAnios }} año{{ candidatos()!.antiguedadAnios === 1 ? '' : 's' }} de antigüedad).
                Elige la que corresponde a este vehículo.
              </p>
            </div>
            ...
            <div class="cand-list">
                  ...
                  <div class="cand-card__body">
                    ...
                    <div class="cand-meta">
                      ...
                      @if (!c.esAntiguedadExacta) {
                        <span class="cand-age-warn">
                          año {{ c.antiguedadDisponible }} (pedido: {{ candidatos()!.antiguedadAnios }})
                        </span>
                      } @else {
                    ...
                    <div class="cand-years">
                      @for (a of c.aniosDisponibles; track a) {
                        <span class="year-pill" [class.year-pill--active]="a === c.antiguedadDisponible">{{ a }}</span>
                      }
                    </div>
                  </div>
                </button>
              ...
              <button type="button" class="cand-card cand-card--manual" (click)="abrirManual()">
                <div class="cand-card__body">
                  ...
                  <p class="cand-sub-small">Ninguna de las opciones anteriores corresponde a este vehículo</p>
                </div>
              </button>
            </div>
            ...
            <div class="cand-disclaimer">
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="6" stroke="#78350F" stroke-width="1.3"/><path d="M6.5 4v3M6.5 9h.01" stroke="#78350F" stroke-width="1.3" stroke-linecap="round"/></svg>
              <span>¿Ves entradas incorrectas o duplicadas? Ve a
            ...
            <div class="bar-wrap">
              <div class="bar">
                <div class="bar-seg bar-seg--base" [style.flex]="props().base" title="Base en pesos"></div>
                <div class="bar-seg bar-seg--imp" [style.flex]="props().imp" title="Impuestos"></div>
                <div class="bar-seg bar-seg--hon" [style.flex]="props().hon" title="Honorarios + extras"></div>
              </div>
              <div class="bar-legend">
                <span><i class="dot dot--base"></i>Base {{ pctOfTotal(r.valorPesos) }}%</span>
                <span><i class="dot dot--imp"></i>Impuestos {{ pctOfTotal(r.impuestosTotal) }}%</span>
                <span><i class="dot dot--hon"></i>Honorarios+ {{ pctOfTotal(r.honorarios + (r.cargoExpress || 0)) }}%</span>
              </div>
            </div>

            <!-- Toggle math -->
            <button type="button" class="math-toggle" (click)="showMath.set(!showMath())">
              <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M1.5 5.5L4.5 8.5L9.5 2.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
            ...
            <div class="lines" [class.lines--math]="showMath()">
              ...
              <div class="line line--base">
                ...
                @if (showMath()) {
                  <span class="line__formula">\${{ r.valorAduanaUsd | number:'1.2-2' }} × {{ r.tipoCambioAplicado?.toFixed(4) }}</span>
                }
              </div>

              <div class="line">
                <div class="line__main">
                  <span class="line__label">IGI <em>{{ (r.igiPorcentaje * 100) | number:'1.0-2' }}%</em></span>
                  <strong class="line__amount">\${{ r.igi | number:'1.2-2' }}</strong>
                </div>
                @if (showMath()) {
                  <span class="line__formula">Valor × {{ (r.igiPorcentaje * 100) | number:'1.0-2' }}%</span>
                }
              </div>
              ...
              <div class="line">
                ...
                @if (showMath()) {
                  <span class="line__formula">
                    @if (r.regimenFiscal === 'POST_2017') { máx(Valor × 0.8%, $202) }
                    @else { Cuota fija }
                  </span>
                }
              </div>
            ...
            <div class="evidence">
              <div class="evidence__head">
                ...
                <span class="match-pill" [class]="'match-pill--' + (r.precioMatchTipo || 'na').toLowerCase()">
                  {{ r.precioMatchTipo || 'SIN DATO' }}
                  @if (r.precioMatchScore) { · {{ r.precioMatchScore }} }
                </span>
              </div>
              <p class="evidence__row">
                <span>Vehículo en catálogo</span>
                <strong>{{ r.precioCatalogoMarca || r.marca || '—' }} {{ r.precioCatalogoModelo || '' }}</strong>
              </p>
              <p class="evidence__row">
              ...
              @if (r.precioAdvertencia) {
                <div class="evidence__warn">
                  <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M6.5 1L12 11H1L6.5 1z" stroke="#B45309" stroke-width="1.4" stroke-linejoin="round"/><path d="M6.5 5.5v2.5M6.5 9.5h.01" stroke="#B45309" stroke-width="1.4" stroke-linecap="round"/></svg>
                  {{ r.precioAdvertencia }}
                </div>
              }
            </div>
            ...
            <div class="empty">
              <div class="empty__icon">
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
                  <rect x="4" y="4" width="28" height="28" rx="4" stroke="#C5C8D6" stroke-width="1.5"/>
                  <path d="M10 18h16M10 24h10M10 12h16" stroke="#C5C8D6" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
              </div>
              <p class="empty__title">Sin cálculo todavía</p>
              <p class="empty__help">Captura el VIN o los datos básicos del vehículo y presiona <em>Calcular cotización</em>.</p>
            </div>
          }
        </aside>
      </div>
  ...
  styles: [`
    ...
    .eyebrow {
      ...
      text-transform: uppercase; color: #8B93A1; margin: 0 0 4px;
    }
    .title { font-size: 26px; font-weight: 700; letter-spacing: -0.5px; color: #0D1017; margin: 0; line-height: 1.1; }

    /* TC chip top right */
    .tc-chip {
      display: flex; flex-direction: column; gap: 4px;
      padding: 12px 16px 10px; border: 1px solid #E4E7EC; border-radius: 14px;
      background: linear-gradient(135deg, #FFFFFF 0%, #F8FAFC 100%);
      min-width: 200px; box-shadow: 0 1px 2px rgba(13,16,23,.04);
    }
    .tc-chip__label {
      font-size: 10.5px; font-weight: 700; letter-spacing: 0.6px;
      text-transform: uppercase; color: #6B717F; display: flex; align-items: center; gap: 6px;
    }
    .tc-dot { width: 7px; height: 7px; border-radius: 50%; background: #D1D5DB; }
    .tc-dot--live { background: #16A34A; box-shadow: 0 0 0 3px rgba(22,163,74,.18); }
    .tc-chip__value {
      font-family: 'JetBrains Mono', monospace; font-size: 22px;
      font-weight: 600; color: #0D1017; line-height: 1; letter-spacing: -0.5px;
    }
    .tc-chip__value small {
      font-family: 'Inter', system-ui; font-size: 10.5px; font-weight: 500;
      color: #8B93A1; margin-left: 6px; letter-spacing: 0.3px;
    }
    .tc-chip__time { font-size: 10.5px; color: #8B93A1; margin: 0; }
    ...
    .cot-form { display: flex; flex-direction: column; gap: 16px; min-width: 0; }

    /* ─── Cards ─────────────────────────── */
    .card {
      background: white; border-radius: 16px; padding: 22px 22px;
      border: 1px solid #ECEFF3;
      box-shadow: 0 1px 3px rgba(13,16,23,.05), 0 0 0 1px rgba(13,16,23,.02);
      transition: box-shadow 200ms cubic-bezier(.16,1,.3,1);
    }
    .card:hover { box-shadow: 0 6px 24px rgba(13,16,23,.06), 0 0 0 1px rgba(13,16,23,.03); }

    .step-pill {
      display: flex; align-items: center; gap: 10px; margin-bottom: 16px;
      font-size: 13.5px; font-weight: 600; color: #1E2330;
    }
    .step-num {
      display: inline-grid; place-items: center;
      width: 22px; height: 22px; border-radius: 50%;
      background: #0D1017; color: white; font-size: 11px; font-weight: 700;
      font-family: 'JetBrains Mono', monospace;
    }
    .ghost-btn {
      margin-left: auto; padding: 4px 10px; border-radius: 7px;
      background: transparent; border: 1px solid #E4E7EC;
      font-size: 11.5px; font-weight: 600; color: #6B717F;
      cursor: pointer; transition: all 140ms cubic-bezier(.16,1,.3,1);
    }
    .ghost-btn:hover { border-color: #C61D26; color: #C61D26; background: #FEF2F2; }
    .ghost-btn--inline { margin-top: 12px; margin-left: 0; }

    /* ─── VIN card ─────────────────────────── */
    .vin-card { padding-bottom: 18px; }
    .vin-card--decoded { border-color: rgba(22,163,74,.30); background: linear-gradient(135deg, #FFFFFF 0%, #F0FDF4 100%); }
    .big-label {
      font-size: 11px; font-weight: 700; letter-spacing: 0.7px;
      text-transform: uppercase; color: #6B717F;
      display: block; margin-bottom: 6px;
    }
    .vin-input-row { display: flex; align-items: center; gap: 10px; }
    .vin-input {
      flex: 1; height: 52px; border-radius: 12px; border: 1.5px solid #E4E7EC;
      background: white; padding: 0 16px;
      font-family: 'JetBrains Mono', monospace;
      font-size: 18px; font-weight: 500; letter-spacing: 1px;
      text-transform: uppercase; color: #0D1017; outline: none;
      transition: border-color 150ms, box-shadow 150ms;
    }
    .vin-input:focus { border-color: #C61D26; box-shadow: 0 0 0 4px rgba(198,29,38,.10); }
    .vin-input::placeholder { color: #C5C8D6; letter-spacing: normal; font-size: 13px; text-transform: none; }
    .vin-counter {
      font-family: 'JetBrains Mono', monospace; font-size: 13px;
      color: #8B93A1; font-weight: 600; min-width: 48px; text-align: right;
      transition: color 140ms;
    }
    .vin-counter--ok { color: #16A34A; }

    .vin-status {
      margin-top: 10px; display: inline-flex; align-items: center; gap: 6px;
      padding: 6px 10px; border-radius: 8px; font-size: 12.5px; font-weight: 500;
    }
    .vin-status--decoding { background: #EFF6FF; color: #1E40AF; }
    .vin-status--ok { background: #F0FDF4; color: #166534; }
    .vin-status--err { background: #FEF2F2; color: #991B1B; }
    .vin-status strong { font-weight: 600; }
    ...
    .vehicle-summary > div { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
    .vehicle-summary span {
      font-size: 10px; font-weight: 700; letter-spacing: 0.5px;
      text-transform: uppercase; color: #8B93A1;
    }
    .vehicle-summary strong { font-size: 13.5px; color: #0D1017; font-weight: 600; }
    ...
    .field input, .field select {
      height: 38px; border-radius: 10px; border: 1px solid #E4E7EC;
      background: white; padding: 0 12px; font-size: 13px; color: #1E2330;
      font-family: 'Inter', system-ui; outline: none;
      transition: border-color 140ms, box-shadow 140ms;
    }
    .field input:focus, .field select:focus { border-color: #C61D26; box-shadow: 0 0 0 3px rgba(198,29,38,.08); }
    .field .readonly {
      height: 38px; line-height: 38px; padding: 0 12px;
      border-radius: 10px; background: #F8FAFC; border: 1px solid #ECEFF3;
      font-family: 'JetBrains Mono', monospace; font-size: 13px;
      font-weight: 600; color: #0D1017;
    }
    .field .hint { font-size: 11px; color: #8B93A1; margin-top: 2px; }
    .advanced {
      margin-top: 14px; padding-top: 14px; border-top: 1px dashed #ECEFF3;
    }

    /* ─── Calculate button ─────────────────────── */
    .calc-row { display: flex; align-items: center; gap: 14px; margin-top: 18px; flex-wrap: wrap; }
    .btn-calc {
      ...
      cursor: pointer; transition: transform 140ms cubic-bezier(.16,1,.3,1), box-shadow 140ms;
      box-shadow: 0 2px 8px rgba(198,29,38,.18), 0 0 0 1px rgba(198,29,38,.10);
    }
    .btn-calc:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(198,29,38,.28); }
    .btn-calc:active:not(:disabled) { transform: translateY(0); }
    .btn-calc:disabled { opacity: 0.38; cursor: not-allowed; }
    .err-msg { color: #991B1B; font-size: 12.5px; margin: 0; font-weight: 500; }
    ...
    .cliente-pop {
      ...
      box-shadow: 0 12px 32px rgba(13,16,23,.10);
    }
    .cliente-row {
      display: block; width: 100%; text-align: left;
      padding: 10px 14px; border: none; background: transparent;
      font-size: 13px; color: #1E2330; cursor: pointer; transition: background 120ms;
    }
    .cliente-row:hover { background: #F8FAFC; }
    .cliente-row__apodo { font-weight: 600; }
    .cliente-row__full { color: #8B93A1; margin-left: 4px; }

    /* ─── Save button ─────────────────────────── */
    .btn-save {
      margin-top: 18px; padding: 11px 22px; border-radius: 12px; border: none;
      background: #16A34A; color: white; font-size: 13.5px; font-weight: 600;
      cursor: pointer; display: inline-flex; align-items: center; gap: 8px;
      transition: transform 140ms, box-shadow 140ms;
    }
    .btn-save:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(22,163,74,.25); }
    ...
    .cot-result {
      ...
      border: 1px solid #ECEFF3;
      box-shadow: 0 8px 32px rgba(13,16,23,.06), 0 0 0 1px rgba(13,16,23,.02);
      max-height: calc(100vh - 96px); overflow-y: auto;
    }
    @media (max-width: 1100px) { .cot-result { position: static; max-height: none; } }

    /* Total card */
    .total-card {
      margin: -6px -6px 16px; padding: 18px 18px 16px;
      border-radius: 14px;
      background: linear-gradient(135deg, #0D1017 0%, #1E2330 100%);
      color: white;
    }
    .total-card .eyebrow { color: rgba(255,255,255,.55); margin-bottom: 6px; }
    .total-number {
      font-family: 'JetBrains Mono', monospace;
      font-size: 32px; font-weight: 700; letter-spacing: -1px; color: white;
      margin: 0; line-height: 1;
    }
    .total-number span {
      font-family: 'Inter', system-ui; font-size: 12px; font-weight: 600;
      color: rgba(255,255,255,.55); margin-left: 8px; letter-spacing: 0.5px;
    }
    .total-meta {
    ...
    .regimen-pill {
      ...
      background: rgba(198,29,38,.20); color: #FCA5A5;
      font-family: 'JetBrains Mono', monospace;
    }
    .fraccion {
      padding: 3px 8px; border-radius: 5px;
    ...
    .bar {
      ...
      background: #F1F3F6; gap: 2px;
    }
    .bar-seg { transition: flex 240ms cubic-bezier(.16,1,.3,1); }
    .bar-seg--base { background: #3B82F6; }
    .bar-seg--imp  { background: #D97706; }
    .bar-seg--hon  { background: #16A34A; }

    .bar-legend {
      display: flex; gap: 12px; margin-top: 8px; font-size: 11px; color: #6B717F;
      flex-wrap: wrap;
    }
    .bar-legend span { display: inline-flex; align-items: center; gap: 5px; }
    .dot { width: 7px; height: 7px; border-radius: 50%; display: inline-block; }
    .dot--base { background: #3B82F6; }
    .dot--imp  { background: #D97706; }
    .dot--hon  { background: #16A34A; }

    /* Math toggle */
    .math-toggle {
      display: inline-flex; align-items: center; gap: 6px;
      margin-bottom: 10px; padding: 6px 10px; border-radius: 8px;
      background: transparent; border: 1px dashed #D8DEE8;
      font-size: 11.5px; font-weight: 600; color: #6B717F;
      cursor: pointer; transition: all 140ms cubic-bezier(.16,1,.3,1);
    }
    .math-toggle:hover { border-style: solid; border-color: #C61D26; color: #C61D26; }
    .math-toggle svg { color: currentColor; }

    /* Line items */
    .lines { display: flex; flex-direction: column; gap: 2px; padding: 4px 0; }
    .line {
    ...
    .line__main { display: flex; justify-content: space-between; align-items: baseline; gap: 12px; }
    .line__label {
      font-size: 12.5px; color: #1E2330; font-weight: 500;
    }
    .line__label em {
      font-style: normal; color: #8B93A1;
      font-family: 'JetBrains Mono', monospace; font-size: 11px;
      font-weight: 600; margin-left: 4px;
    }
    .line__amount {
      font-family: 'JetBrains Mono', monospace;
      font-size: 13px; color: #0D1017; font-weight: 600;
      letter-spacing: -0.3px;
    }
    .line__formula {
      font-family: 'JetBrains Mono', monospace; font-size: 10.5px;
      color: #8B93A1; padding-left: 0; line-height: 1.3;
      letter-spacing: 0;
    }
    .lines--math .line { padding: 8px; }

    .line--base { background: #EFF6FF; }
    .line--base .line__label { color: #1E40AF; font-weight: 600; }
    .line--base .line__amount { color: #1E40AF; }

    .line--subtotal {
      margin-top: 4px; padding-top: 9px;
      border-top: 1px solid #ECEFF3;
    }
    .line--subtotal .line__label { font-weight: 700; color: #0D1017; }
    .line--subtotal .line__amount { color: #0D1017; font-weight: 700; }

    .line--total {
      margin-top: 6px; padding: 11px 8px;
      background: linear-gradient(135deg, rgba(198,29,38,.06) 0%, rgba(198,29,38,.02) 100%);
      border: 1px solid rgba(198,29,38,.16); border-radius: 9px;
    }
    .line--total .line__label { font-size: 13px; font-weight: 700; color: #0D1017; letter-spacing: 0.4px; text-transform: uppercase; }
    .line--total .line__amount { font-size: 15px; color: #C61D26; font-weight: 700; }

    /* Evidence */
    .evidence {
      margin-top: 14px; padding: 14px;
      background: #FAFBFC; border: 1px solid #ECEFF3; border-radius: 12px;
    }
    .evidence__head {
    ...
    .evidence__row {
      ...
      padding: 4px 0; margin: 0; font-size: 11.5px;
    }
    .evidence__row span { color: #8B93A1; }
    .evidence__row strong { color: #1E2330; font-weight: 600; text-align: right; }
    .evidence__warn {
    ...
    .match-pill--especifico { background: #F0FDF4; color: #166534; }
    .match-pill--marca { background: #FFFBEB; color: #78350F; }
    .match-pill--generico { background: #FEF2F2; color: #991B1B; }
    .match-pill--override { background: #F5F3FF; color: #5B21B6; }
    .match-pill--amparo { background: #EFF6FF; color: #1E40AF; }
    .match-pill--na { background: #F1F3F6; color: #6B717F; }

    /* Empty state */
    .empty {
      padding: 60px 20px; text-align: center;
    }
    .empty__icon { display: inline-grid; place-items: center; margin-bottom: 14px; opacity: 0.7; }
    .empty__title { font-size: 14.5px; font-weight: 600; color: #1E2330; margin: 0 0 4px; }
    .empty__help { font-size: 12.5px; color: #8B93A1; margin: 0; line-height: 1.5; }
    .empty__help em { color: #C61D26; font-style: normal; font-weight: 600; }

    /* Loading state */
    .result-loading { padding: 20px 4px; }
    .loader-bar { margin: 14px 0 12px; height: 6px; background: #F1F3F6; border-radius: 3px; overflow: hidden; }
    .loader-bar__fill {
      width: 40%; height: 100%; background: linear-gradient(90deg, #C61D26, #FB7185);
      border-radius: 3px;
      animation: loader-slide 1.4s cubic-bezier(.16,1,.3,1) infinite;
    }
    @keyframes loader-slide { 0% { margin-left: -40%; } 100% { margin-left: 100%; } }
    .loader-stage { font-size: 14px; font-weight: 600; color: #0D1017; margin: 0 0 6px; }
    .loader-help { font-size: 12px; color: #8B93A1; margin: 0; line-height: 1.5; }

    /* ─── Candidatos picker ────────────────────── */
    .cand-header { margin-bottom: 14px; }
    .cand-title {
      font-size: 15px; font-weight: 700; color: #0D1017;
      margin: 4px 0 6px; letter-spacing: -0.3px;
    }
    .cand-sub {
      font-size: 12px; color: #6B717F; margin: 0; line-height: 1.5;
    }
    .cand-sub strong { color: #0D1017; font-weight: 600; }

    .cand-list { display: flex; flex-direction: column; gap: 8px; margin-bottom: 12px; }

    .cand-card {
      width: 100%; text-align: left; border: 1.5px solid #E4E7EC;
      border-radius: 12px; padding: 0; background: white;
      cursor: pointer; transition: all 150ms cubic-bezier(.16,1,.3,1);
      overflow: hidden;
    }
    .cand-card:hover { border-color: #C61D26; box-shadow: 0 4px 14px rgba(198,29,38,.10); }
    .cand-card--suggested { border-color: rgba(22,163,74,.40); background: #F0FDF4; }
    .cand-card--suggested:hover { border-color: #16A34A; box-shadow: 0 4px 14px rgba(22,163,74,.14); }
    .cand-card--manual { background: #F8FAFC; border-style: dashed; }
    .cand-card--manual:hover { border-color: #6B717F; border-style: solid; background: white; }

    .cand-card__top {
      display: flex; align-items: center; gap: 6px;
      padding: 7px 12px 6px;
      background: rgba(13,16,23,.03); border-bottom: 1px solid #F1F3F6;
      flex-wrap: wrap;
    }
    .cand-frac {
      font-family: 'JetBrains Mono', monospace; font-size: 10.5px;
      font-weight: 700; color: #1E2330; letter-spacing: 0.2px;
    }
    .cand-origen {
      font-size: 10px; color: #8B93A1; font-weight: 500;
      padding: 2px 6px; background: #ECEFF3; border-radius: 4px;
    }
    .cand-badge {
    ...
    .cand-card__body { padding: 10px 12px 10px; }
    .cand-modelo {
      font-size: 13px; font-weight: 600; color: #1E2330; margin: 0 0 6px;
    }
    .cand-meta {
    ...
    .cand-age-ok {
      ...
      padding: 2px 6px; background: #DCFCE7; border-radius: 4px;
    }
    .cand-age-warn {
      font-size: 10.5px; font-weight: 600; color: #92400E;
      padding: 2px 6px; background: #FEF3C7; border-radius: 4px;
    }
    .cand-years { display: flex; gap: 4px; flex-wrap: wrap; }
    .year-pill {
      font-family: 'JetBrains Mono', monospace; font-size: 9.5px;
      padding: 2px 5px; border-radius: 4px;
      background: #F1F3F6; color: #6B717F; font-weight: 600;
    }
    .year-pill--active { background: #DBEAFE; color: #1E40AF; }
    .cand-sub-small { font-size: 11px; color: #8B93A1; margin: 2px 0 0; }

    .cand-disclaimer {
      display: flex; gap: 7px; align-items: flex-start;
      padding: 10px 12px; border-radius: 9px;
      background: #FFFBEB; border: 1px solid rgba(217,119,6,.20);
      font-size: 11px; color: #78350F; line-height: 1.5;
    }
    .cand-disclaimer a { color: #B45309; font-weight: 600; text-decoration: underline; }
    .cand-disclaimer a:hover { color: #78350F; }

    /* Spinner */
    .spinner {
      width: 12px; height: 12px; border: 1.6px solid #E4E7EC;
      border-top-color: #C61D26; border-radius: 50%;
      display: inline-block;
      animation: spin 0.7s linear infinite;
    }
    .spinner--white { border-color: rgba(255,255,255,.4); border-top-color: white; }
...
export class CotizacionNuevaComponent {
  ...
  abrirManual(): void {
    ...
    setTimeout(() => {
      document.querySelector<HTMLElement>('.advanced')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }, 100);
  }
