  ...
  template: `
    <div>
      ...
      <div class="flex items-center gap-3 mb-5 flex-wrap">
        <input [(ngModel)]="search" (input)="buscar()" placeholder="Buscar folio, VIN o modelo"
          class="w-full max-w-[320px] px-3 py-2.5 rounded-xl border border-[#E4E7EC] bg-white text-[13px] outline-none focus:border-[#C61D26]" />
        <select [(ngModel)]="estado" (change)="buscar()" class="px-3 py-2.5 rounded-xl border border-[#E4E7EC] bg-white text-[13px]">
        ...
        @if (loading()) {
              ...
              <tbody>
                @for (c of items(); track c.id) {
                  <tr (click)="router.navigate(['/cotizaciones', c.id])" class="border-b border-[#F3F4F6] hover:bg-[#FAFBFC] cursor-pointer text-[13.5px]">
                    ...
                    <td class="px-5 py-3 text-[12px]" [style.color]="expirada(c) ? '#B0181F' : '#9EA3AE'">
          ...
          @if (totalPages() > 1) {
            <div class="flex items-center justify-between px-5 py-3 border-t border-[#E4E7EC]">
              <span class="text-[12.5px] text-[#9EA3AE]">Página {{ page() }} de {{ totalPages() }} · {{ total() }} registros</span>
