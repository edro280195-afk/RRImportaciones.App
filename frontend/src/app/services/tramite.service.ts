...
export class TramiteService {
  ...
  uploadEvidence(file: File): Observable<{ url: string }> {
    ...
    return this.http.post<{ url: string }>(`${this.baseUrl}/upload-evidencia`, formData);
  }

  uploadDocumento(file: File): Observable<{ url: string }> {
    const formData = new FormData();
    formData.append('file', file);
    return this.http.post<{ url: string }>(`${this.baseUrl}/upload-documento`, formData);
  }
}
