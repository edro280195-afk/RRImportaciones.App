...
export class PortalService {
  ...
  private readonly baseUrl = environment.apiUrl + '/api/portal';

  getTramite(token: string): Observable<PortalTramiteDto> {
    return this.http.get<PortalTramiteDto>(`${this.baseUrl}/acceso/${token}`);
  }

  generateToken(tramiteId: string): Observable<{ token: string }> {
    return this.http.get<{ token: string }>(`${this.baseUrl}/generar-token/${tramiteId}`);
  }
}
