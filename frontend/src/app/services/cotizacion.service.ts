...
export interface CotizacionInput {
  ...
  /** ID de la entrada del catálogo elegida manualmente por el admin en el paso de candidatos */
  precioEstimadoIdOverride: string | null;
  /** Solo para régimen AMPARO: "NORMAL" | "LUJO". LUJO nunca se auto-detecta. */
  categoriaAmparoOverride: string | null;
  tcMargen: number;
  tipoTramite: string;
  honorariosOverride: number | null;
}

export interface CandidatoPrecio {
  precioEstimadoId: string;
  fraccion: string;
  modeloCatalogo: string;
  marcaTextoCatalogo: string;
  hojaOrigen: string;
  matchTipo: string;
  score: number;
  antiguedadDisponible: number;
  esAntiguedadExacta: boolean;
  precioUsd: number;
  esSugerido: boolean;
...
export interface CotizacionOutput {
  ...
  categoriaAmparoUsada: string | null;
  fuentePrecio: string;
  ...
  tipoCambioContexto: string | null;
...
export class CotizacionService {
  ...
  private readonly baseUrl = environment.apiUrl + '/api/cotizaciones';
  ...
  obtenerCandidatos(input: CotizacionInput): Observable<CandidatosPrecioOutput> {
    return this.http.post<CandidatosPrecioOutput>(`${this.baseUrl}/candidatos`, input);
  }
  ...
  getTipoCambio(contexto: string = 'FIX'): Observable<TipoCambioDto> {
    return this.http.get<TipoCambioDto>(`${this.baseUrl}/tipo-cambio`, { params: { contexto } });
  }
